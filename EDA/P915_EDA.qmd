...
p915_biolnew1 <- read.csv("Data/P915/Finalized/p915_biol1new.csv")
p915_biolnew2 <- read.csv("Data/P915/Finalized/p915_biol2new.csv")
p915_bioln <- rbind(p915_biolnew1, p915_biolnew2)

#Edit CPUE dataset, can put this in dataset cleaning file 
...
```{r}
head(p915_bioln)
sci_spp <- as.data.frame(unique(p915_bioln$Species))
names(sci_spp)[names(sci_spp) == "unique(p915_bioln$Species)"] <- "Sci_nm"
speicescommon <- as.data.frame(c("southern flounder", "red drum", "northern kingfish", "blue crab", "atlantic croaker", "black drum", "atlantic sharpnose", "bonnethead hammerhead", "sheepshead", "brown shrimp", "southern kingfish", "white shrimp", "eastern oyster", "sandbar shark", "oyster toadfish", "bull shark", "blue catfish", "NA", "blacktip shark", "scalloped hammerhead"))
colnames(speicescommon)[1] = "Speciescommoname"
speicescommon %>% mutate(X= 1:20) %>% left_join(sci_spp %>% mutate(X= 1:20), by = "X")

unique(p915_bioln$Species)
p915_bioln_wt <- p915_bioln %>% group_by(Species, Latitude, Longitude, Year, Month, Ym_date) %>% summarise(Weight= mean(Colwgt))

cent_fish_nc <- p915_bioln_wt %>% group_by(Year, Month, Species) %>% 
...
cent_fish_nc %>% dplyr::filter(Species == "C. sapidus") %>% ggplot() + geom_sf(data = world) + geom_point(aes(x = lon, y = lat, colour= Year), size = 1) + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + facet_wrap(~Month)

unique(p915_bioln$Species)
cent_fish_nc %>% dplyr::filter(Species == "I. furcatus") %>% ggplot() + geom_sf(data = world) + geom_point(aes(x = lon, y = lat, colour= Year), size = 1) + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + facet_wrap(~Month)

cent_fish_nc %>% dplyr::filter(Species == "S. tiburo") %>% ggplot() + geom_sf(data = world) + geom_point(aes(x = lon, y = lat, colour= Year), size = 1) + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + facet_wrap(~Month)
