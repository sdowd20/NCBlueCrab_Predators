...
```{r}
biol_clean <- read_excel("/Users/sallydowd/Desktop/Ch1Data/P135/biol_edt.xlsx", col_types= "text")
...
```{r}
length(unique(habitat$Control1)) #121,743
length(unique(biol$Control1)) #71,946, only has Control1 for fish captured

#Create CPUE dataset with biol
summary(is.na(biol))

biol_edt <- biol %>%

#distinct Control3s for a sampling event (Control1, Species), one row for each species caught in a net
#do it on example data first 
#after, link the habitat and biological datasets, see P120 
```
