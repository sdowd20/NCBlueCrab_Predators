...
control_spp <- p915_CPUE %>% filter(Year >= 2001) %>% group_by(Control1, Speciescommonname) %>% summarize(count = n())
unique(control_spp$count)
...
t <- unique(p915_bioln$Sample)
t2 <- unique(p915_CPUE$Sample)

differences <- anti_join(p915_CPUE %>% filter(Year >= 2001), p915_bioln %>% filter(Year >= 2001), by = c("Sample" = "Sample"))
unique(differences$Speciescommonname)
```

```{r}
length <- read_excel("/Users/sallydowd/Desktop/P915LengthData_20231003.xlsx", sheet=1)

t <- length %>% filter(SAMPLE %in% "1035HYDE320.")
write.csv(t, "~/Desktop/length.example.csv")
t2 <- p915_CPUE %>% filter(Sample %in% "1035HYDE320.", Speciescommonname %in% "atlantic menhaden")
write.csv(t2, "~/Desktop/CPUE.example.csv")
```
...
#Length updated: 10/26/23
```{r}
#Compare w/ CPUE dataset
unique(p915_length$Sample) #13955
unique(p915_CPUE$Sample) #13947
unique(p915_length$Control1) #13947
unique(p915_CPUE$Control1) #20482

merged_df <- merge(p915_CPUE, p915_length, by = "Sample")
length(unique(merged_df$Sample)) #13720 of the same ones 
unmerged_df <- anti_join(p915_CPUE, p915_length, by = "Sample")
length(unique(unmerged_df$Sample)) #227 of different ones, 7945 observations, species include
sample_combo <- merged_df$Sample
yep <- p915_length %>% filter(!Sample %in% sample_combo)
...
tryit <- p915_length %>% filter(Sample %in% "1035HYDE320.", Speciescommonname %in% "atlantic menhaden") %>% dplyr::select(Sample, Speciescommonname, Colnum:Length)
colnum <- p915_CPUE %>% filter(Speciescommonname %in% "atlantic menhaden") %>% select(Sample, Colnum) %>% distinct(Sample, .keep_all = TRUE)
...
length_freq %>% summarize(sum= sum(Number)) #865
length_freq %>% summarize(sum= sum(Number2)) #867

#Red drum example
tryit2 <- p915_length %>% filter(Speciescommonname %in% "red drum", Sample %in% "7515DARE120.") %>% dplyr::select(Sample, Speciescommonname, Colnum:Length)

p915_length %>% filter(Speciescommonname %in% "red drum", Sample %in% "7515DARE120.") %>% ggplot() + geom_histogram(aes(x= Length))
#Deal with this differently, show example to Janet 

p915_CPUE %>% filter(Speciescommonname %in% "red drum", Colnum > 0) %>% summarize(uni= length(unique(Sample))) 
p915_length %>% filter(Speciescommonname %in% "red drum") %>% summarize(uni= length(unique(Sample)))

t <- p915_length %>% filter(Speciescommonname %in% "red drum") %>% select(Sample) %>% left_join(p915_CPUE %>% filter(Speciescommonname %in% "red drum", Colnum > 0) %>% select(Sample, Colnum))


p915_length %>% drop_na(Length) %>% group_by(Speciescommonname) %>% ggplot() + geom_histogram(aes(x= Length, y= after_stat(count)), bins= 20) + facet_wrap(~Speciescommonname, scale= "free") + xlab("Length (mm)") + ylab("Count") + standard_theme + xlim(c(0,1000))
```
