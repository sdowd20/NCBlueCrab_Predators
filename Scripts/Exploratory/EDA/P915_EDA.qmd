...
Here are what the variable names mean: 
-Sample Number: number of particular species in sampling unit -Colnum: total number of a particular species in that sample, code as 9s for species where count can't be determined, multiple numbers in dataset as it is broken up by mesh size 
-Line number: each record 4 type numbered sequentially, record type 4 means that biological data besides actual lengths are recorded, the # of each length, so if five lengths from individuals in a given mesh size are collected it's #ed 1-5, if we collected 12 it would be numbered 1-12
-Frequency: Number of occurrences for like data. In the length dataset, this always = 1. 
-Length (mm): Individual length from subsample, menhaden is typically the only species that is subsampled, other species that sometimes are subsampled are spot, bluefish, and white perch (if there are large catches), subsample at the net (mesh size) not at the sample level. 
...
#New length
p915_length <- read.csv("/Users/sallydowd/Desktop/Ch1Data/P915/p915_length_new.csv")
p915_length <- p915_length[,-1]

t <- unique(p915_length$Sample) #13955
t2 <- unique(p915_CPUE$Sample) #13947
tC <- unique(p915_length$Control1)
t2C <- unique(p915_CPUE$Control1) #do not use Control1 for length- it is weird 
...
unique(yep$Sample)
unique(unmerged_df$Sample)

write.csv(unmerged_df, "~/Desktop/CPUE_different.csv")
write.csv(yep, "~/Desktop/length_different.csv")
```
