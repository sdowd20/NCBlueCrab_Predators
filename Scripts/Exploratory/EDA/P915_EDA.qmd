...
Here are what the variable names mean: -Sample Number: number of particular species in sampling unit -Colnum: total number of a particular species in the catch, code as 9s for species where count can't be determined -Line number: each record 4 type numbered sequentially, record type 4 means that biological data besides actual lengths are recorded -Frequency: Number of occurrences for like data. In the length dataset, this always = 1. -Length (mm): Individual length from subsample
