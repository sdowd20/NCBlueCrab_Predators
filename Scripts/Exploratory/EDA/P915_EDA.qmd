...
tryit2 <- p915_len_f %>% filter(Speciescommonname %in% "atlantic menhaden") %>% dplyr::select(Sample, Speciescommonname, Colnum,Length, Linenum)
tryit2$Age_class <- ifelse(tryit2$Length <= 250, "J", "A") #assign age class
test2 <- tryit2 %>% group_by(Sample, Colnum, Age_class) %>% summarize(count= n()) %>% mutate(Freq= count/sum(count), Number= Freq*Colnum) #counted # of observations in each age class for a sample, computed a frequency, multiplied frequency by total Colnum
test2_edt <- test2 %>% group_by(Sample, Age_class) %>% summarize(Num_sum = sum(Number)) #sum number of individuals in each age class, 10797 samples 
...
unique(differences$Speciescommonname) 
 ...
 t <- tryit2 %>% filter(Sample %in% "631PAMLI2300.")
