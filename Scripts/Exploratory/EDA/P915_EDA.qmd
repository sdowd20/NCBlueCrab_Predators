...
tryit <- p915_len_f %>% filter(Sample %in% "1035HYDE320.", Speciescommonname %in% "atlantic menhaden") %>% dplyr::select(Sample, Speciescommonname, Colnum,Length, Linenum)

tryit$Age_class <- ifelse(tryit$Length <= 250, "J", "A")
test <- tryit %>% group_by(Sample, Colnum, Age_class) %>% summarize(count= n())



t <- p915_CPUE %>% filter(Sample %in% "1035HYDE320"), Speciescommonname %in% "atlantic menhaden")
