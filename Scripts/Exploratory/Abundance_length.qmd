...
p915_len_f %>% drop_na(Length) %>% filter(Speciescommonname %in% interest_spp) %>% group_by(Speciescommonname) %>% ggplot() + geom_histogram(aes(x= Length)) + facet_wrap(~Speciescommonname, scales= "free") + xlab("Length (mm)") + ylab("Count") + standard_theme
