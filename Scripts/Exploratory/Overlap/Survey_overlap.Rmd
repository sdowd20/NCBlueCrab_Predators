...
p915_overlap <- p915 %>% dplyr::select(Date, Year, Month, Area, Ssal, Stemp, Sdo, Speciescommonname, Colnum, Latitude, Longitude, Sciname, Season) %>% mutate(Survey= "P915", lat_lon= paste(Latitude, Longitude, sep= "_")) %>% drop_na(Latitude, Longitude) %>% filter(Year > 2008)
p120_overlap <- p120 %>% dplyr::select(Date, Year, Month, Station, Stemp, Ssal, Sdo, Speciescommonname, Colnum, Latitude, Longitude, Sciname, Season) %>% dplyr::rename("Area" = "Station") %>% mutate(Survey= "P120", lat_lon= paste(Latitude, Longitude, sep= "_")) %>% drop_na(Latitude, Longitude) %>% filter(Year > 2008)
p195_overlap <- p195 %>% dplyr::select(Date, Year, Month, Location, Tempsurface, Salinitysurface, Sdo, Speciescommonname, Numbertotal, Latitudestart, Longitudestart, Sciname, Season) %>% dplyr::rename("Area"= "Location", "Stemp"= "Tempsurface", "Ssal"= "Salinitysurface", "Latitude"= "Latitudestart", "Longitude"= "Longitudestart", "Colnum"= "Numbertotal") %>% mutate(Survey= "P195", lat_lon= paste(Latitude, Longitude, sep= "_")) %>% drop_na(Latitude, Longitude) %>% filter(Year > 2008)
...
ggplot(data = world) + geom_sf() + geom_sf(data = globe_grid_20, fill = NA) + geom_point(data = subset(p195, p195$Speciescommonname %in% "blue crab" & as.numeric(p195$Numbertotal > 0)), aes(x = as.numeric(Longitudestart), y = as.numeric(Latitudestart), size= Numbertotal), colour= "orange") + geom_point(data = subset(p120, p120$Speciescommonname %in% "blue crab" & as.numeric(p120$Colnum > 0)), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), size= Colnum), colour= "pink") + geom_point(data = subset(p915, p915$Speciescommonname %in% species & as.numeric(p915$Colnum > 0)), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), size= Colnum), colour= "blue") + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black")) + labs(x= "Longitude", y= "Latitude") + guides(size= F, scale= "none") + facet_wrap(~Season)
}
...
ggplot(data = world) + geom_sf() + geom_sf(data = globe_grid_20, fill = NA) + geom_point(data = subset(p195, p195$Speciescommonname %in% "blue crab" & as.numeric(p195$Numbertotal == 0)), aes(x = as.numeric(Longitudestart), y = as.numeric(Latitudestart), size= Numbertotal), colour= "orange") + geom_point(data = subset(p120, p120$Speciescommonname %in% "blue crab" & as.numeric(p120$Colnum == 0)), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), size= Colnum), colour= "red") + geom_point(data = subset(p915, p915$Speciescommonname %in% species & as.numeric(p915$Colnum > 0)), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), size= Colnum), colour= "blue") + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black")) + labs(x= "Longitude", y= "Latitude")+ guides(size= F, scale= "none") + facet_wrap(~Season)
}
...
ggplot(data = world) + geom_sf() + geom_sf(data = globe_grid_20, fill = NA) + geom_point(data = subset(p120, p120$Speciescommonname %in% "blue crab" & as.numeric(p120$Colnum > 0)), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), size= Colnum), colour= "red") + geom_point(data = subset(p915, p915$Speciescommonname %in% species & as.numeric(p915$Colnum == 0)), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), size= Colnum), colour= "blue") + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black")) + labs(x= "Longitude", y= "Latitude")+ guides(size= F, scale= "none") + facet_wrap(~Season)
}
```
...
ggplot(data = world) + geom_sf() + geom_point(data = subset(p120_edt, p120_edt$Speciescommonname %in% species), aes(x = as.numeric(Longitude), y = as.numeric(Latitude), colour = Colnum)) + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black")) + labs(x= "Longitude", y= "Latitude", colour= "# caught") + ggtitle(species)
}
...
total_edt <- total_edt %>% filter(Speciescommonname %in% select_spp, !Area %in% "CAPEF", !Area %in% "NEWR", Year >= 2008, Month == 5| Month== 6| Month== 9, Longitude > -77.0) %>% drop_na(Latitude, Longitude) #NA coordinates already dropped

p915_overlap_edt <- p915_overlap %>% filter(Speciescommonname %in% select_spp) %>% filter(Year >= 2008, Month == 5| Month== 6| Month== 9)
p120_overlap_edt <- p120_overlap %>% filter(Speciescommonname %in% select_spp) %>% filter(Year >= 2008, Month == 5| Month== 6| Month== 9)
p195_overlap_edt <- p195_overlap %>% filter(Speciescommonname %in% select_spp) %>% filter(Year >= 2008, Month == 5| Month== 6| Month== 9)
...
###By season:
p915_overlap_gridIDs <- total_edt %>% select(gridID, Survey, Season, Month) %>% filter(Survey %in% "P915") %>% group_by(Month) %>% distinct(gridID, .keep_all= TRUE) %>% select(gridID, Season)
p120_overlap_gridIDs <- total_edt %>% select(gridID, Survey, Season, Month) %>% filter(Survey %in% "P120") %>% group_by(Month) %>% distinct(gridID, .keep_all= TRUE) %>% select(gridID, Season)
p195_overlap_gridIDs <- total_edt %>% select(gridID, Survey, Season, Month) %>% filter(Survey %in% "P195") %>% group_by(Month) %>% distinct(gridID, .keep_all= TRUE) %>% select(gridID, Season)
library(plyr)
match_df(p120_overlap_gridIDs %>% filter(Month == 6) %>% select(gridID), p915_overlap_gridIDs %>% filter(Month == 6) %>% select(gridID)) #tested with season and results were very similar 
...
detach(package:plyr)
library(dplyr)
fish_grid_m <- total_edt_month %>% group_by(Year, Month, Speciescommonname, Survey, gridID) %>% summarise(unique_fish = n_distinct(Colnum)) %>% pivot_wider(names_from = Survey, values_from = unique_fish) #n_distinct: counts unique combinations, how many times a fish was sampled in a grid cell 
#write.csv(fish_grid_m, "~/Desktop/fish_grid_m.csv")

#Grid, Season 
fish_grid_s <- total_edt_seas %>% group_by(Year, Season, Speciescommonname, Survey, gridID) %>% summarise(unique_fish = n_distinct(Colnum)) %>% pivot_wider(names_from = Survey, values_from = unique_fish)
#write.csv(fish_grid_s, "~/Desktop/fish_grid_s.csv")
```
...
ggplot() + geom_sf(data = world) + geom_point(data = subset(total_edt, total_edt$Species %in% "bonnethead hammerhead" & Colnum > 0 & Survey %in% "P915"), aes(x = Longitude, y = Latitude), colour= "light green") + geom_point(data = subset(total_edt, total_edt$Species %in% "blue crab" & Colnum > 0 & Survey %in% "P195"), aes(x = Longitude, y = Latitude), colour= "dark green") + geom_point(data = subset(total_edt, total_edt$Species %in% "blue crab" & Colnum > 0 & Survey %in% "P120"), aes(x = Longitude, y = Latitude), colour= "yellow") + scale_size_area() + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black"))
```
...
P915_total_sum <- P915_total %>% filter(Colnum > 0) %>% dplyr::group_by(Speciescommonname, Year, Season, gridID) %>% summarise(sum_Colnum_pred = sum(Colnum)) #add up number of predators based on year, season and gridID
...
match_data2edt <- match_data2 %>% left_join(P915_total_sum) %>% left_join(P120_bc_sum, by= c("Year", "Season", "gridID")) %>% rename(Species= Speciescommonname)
...
##p120 first
pos_zero2 <- bc_pos %>% left_join(p915_zero, by = c("Year", "Season", "gridID")) %>% drop_na(Speciescommonname) %>% rename(Species= Speciescommonname)
...
zero_pos <- p915_pos %>% left_join(bc_zero, by= c("Year", "Season", "gridID")) %>% drop_na(Colnum) %>% rename(Species= Speciescommonname) #exclude when blue crabs weren't sampled
##p120 first 
zero_pos2 <- bc_zero %>% left_join(p915_pos, by= c("Year", "Season", "gridID")) %>% drop_na(Speciescommonname) %>% rename(Species= Speciescommonname)  #exclude when blue crabs weren't sampled
...
match_data2edt %>% group_by(Species, Year) %>% summarize(sum_spp = sum(sum_Colnum_pred), sum_bc = sum(sum_Colnum_bc, na.rm= T)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/pospos.year.pdf")
...
pos_zero %>% group_by(Species) %>% summarize(sum_spp = sum(Colnum), sum_bc = sum(bc_colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/poszero.png")
pos_zero %>% group_by(Species, Year) %>% summarize(sum_spp = sum(Colnum), sum_bc = sum(bc_colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/poszero.year.pdf")
pos_zero %>% group_by(Species, Year, Season) %>% summarize(sum_spp = sum(Colnum), sum_bc = sum(bc_colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/poszero.season.pdf")
pos_zero2 %>% group_by(Species) %>% summarize(sum_spp = sum(Colnum), sum_bc = sum(bc_colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/poszero2.png")
pos_zero2 %>% group_by(Species, Year) %>% summarize(sum_spp = sum(Colnum), sum_bc = sum(bc_colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/poszero2.year.pdf")
pos_zero2 %>% group_by(Species, Year, Season) %>% summarize(sum_spp = sum(Colnum), sum_bc = sum(bc_colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/poszero2.season.pdf")

##Zero blue crab in P120, positive predator in P915
zero_pos %>% group_by(Species) %>% summarize(sum_spp = sum(pred_colnum), sum_bc = sum(Colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/zero.pos.png")
zero_pos %>% group_by(Species,Year) %>% summarize(sum_spp = sum(pred_colnum), sum_bc = sum(Colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/zero.pos.year.pdf")
zero_pos %>% group_by(Species,Year, Season) %>% summarize(sum_spp = sum(pred_colnum), sum_bc = sum(Colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/zero.pos.season.pdf")
zero_pos2 %>% group_by(Species) %>% summarize(sum_spp = sum(pred_colnum), sum_bc = sum(Colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/zero.pos2.png")
zero_pos2 %>% group_by(Species,Year) %>% summarize(sum_spp = sum(pred_colnum), sum_bc = sum(Colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/zero.pos2.year.pdf")
zero_pos2 %>% group_by(Species,Year, Season) %>% summarize(sum_spp = sum(pred_colnum), sum_bc = sum(Colnum)) %>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/zero.pos2.season.pdf")
```

###### Overlap plots: 

```{r}
total_edt_new <- total_edt %>% filter(Speciescommonname %in% select_spp, !Area %in% "CAPEF", !Area %in% "NEWR", Year >= 2008, Season %in% "Spring"| Season %in% "Summer", Longitude > -77.0, Month== 5|Month==6|Month==9)
...
ggsave("/users/sallydowd/desktop/pospos.jpg", width=25, height= 25, units= c("cm")) 

#pos, zero: tried w/ pos_zero2 as well (no big differences)
pos_zero %>% ggplot() + geom_histogram(aes(x= Year)) + facet_wrap(~Species + Season) + standard_theme
ggsave("/users/sallydowd/desktop/poszero.jpg", width=25, height= 25, units= c("cm")) 

#zero, pos: same graph no matter how left_join
zero_pos %>% ggplot() + geom_histogram(aes(x= Year)) + facet_wrap(~Species + Season) + standard_theme
ggsave("/users/sallydowd/desktop/zeropos.jpg", width=25, height= 25, units= c("cm")) 
...
#### P915 & P195: Year > 2008, Month= 5,6,9, group_by Season
```{r}
select_spp2 <- c("atlantic croaker", "black drum", "blue catfish", "bonnethead hammerhead", "red drum", "sheepshead", "southern flounder", "southern kingfish", "striped bass")
...
P915_total_spp[[i]] <- P915_total %>% filter(Speciescommonname %in% i, Colnum > 0) %>% drop_na(Colnum)
P915_total_gridID[[i]] <- P915_total_spp[[i]] %>% dplyr::select(gridID, Season, Speciescommonname, Year) %>% group_by(Season, Year, Speciescommonname) %>% distinct(gridID, .keep_all= TRUE)
P195_bc <- P195_total %>% filter(Speciescommonname %in% "blue crab", Colnum > 0) %>% drop_na(Colnum)
P195_bc_gridID <- P195_bc %>% dplyr::select(gridID, Season, Speciescommonname, Year) %>% group_by(Season, Year, Speciescommonname) %>% distinct(gridID, .keep_all= TRUE)
match_df[[i]] <- match_df(P915_total_gridID[[i]], P195_bc_gridID, on = c("gridID", "Season", "Year")) 
}
...
match_data2 <- match_data2 %>% group_by(Speciescommonname, Season, Year) %>% dplyr::mutate(count= n()) #%>% select(-gridID) #count: number of observations overlapping for a predator species and blue crab for a season and year (includes all grid cells)
P915_total_sum <- P915_total %>% filter(Colnum > 0) %>% dplyr::group_by(Speciescommonname, Year, Season, gridID) %>% summarise(sum_Colnum_pred = sum(Colnum)) #add up number of predators based on year, season and gridID
P195_bc_sum <- P195_total %>% filter(Colnum > 0, Speciescommonname %in% "blue crab") %>% dplyr::group_by(Year, Season, gridID) %>% summarise(sum_Colnum_bc = sum(Colnum))

match_data2edt <- match_data2 %>% left_join(P915_total_sum) %>% left_join(P195_bc_sum, by= c("Year", "Season", "gridID")) %>% rename(Species= Speciescommonname)

#pos, zero
##p915 first 
bc_pos <- P195_total %>% filter(Colnum > 0, Speciescommonname %in% "blue crab") %>% group_by(gridID, Season, Year) %>% mutate(bc_colnum= sum(Colnum)) %>% distinct(bc_colnum, .keep_all= F)
p915_zero <- P915_total %>% filter(Colnum == 0, Speciescommonname %in% select_spp2) %>% drop_na(Colnum) %>% dplyr::select(Year, Speciescommonname, Season, Colnum, gridID) %>% group_by(Season, Year, gridID, Speciescommonname) %>% distinct(Colnum) #only one row per species, gridID, season, and year b/c colnum= 0 
p915_zeroedt <- p915_zero %>% left_join(bc_pos, by = c("Year", "Season", "gridID")) #it's only gridIDs that overlap bc is the ones present in P915, NA for bc_colnum means it wasn't caught there so remove
pos_zero <- p915_zeroedt %>% drop_na(bc_colnum) %>% rename(Species= Speciescommonname)
...
#zero, pos
##p915 first
bc_zero <- P195_total %>% filter(Colnum == 0, Speciescommonname %in% "blue crab") %>% drop_na(Colnum) %>% dplyr::select(Year, Speciescommonname, Season, Colnum, gridID) %>% group_by(gridID, Season, Year) %>% distinct(Colnum, .keep_all= F)
p915_pos <- P915_total %>% filter(Colnum > 0, Speciescommonname %in% select_spp2) %>% group_by(gridID, Season, Year, Speciescommonname) %>% mutate(pred_colnum = sum(Colnum)) %>% distinct(pred_colnum, .keep_all= F) #just need to get one row for a grid cell, season, year and species
zero_pos <- p915_pos %>% left_join(bc_zero, by= c("Year", "Season", "gridID")) %>% drop_na(Colnum) %>% rename(Species= Speciescommonname) #exclude when blue crabs weren't sampled
##p120 first 
zero_pos2 <- bc_zero %>% left_join(p915_pos, by= c("Year", "Season", "gridID")) %>% drop_na(Speciescommonname) %>% rename(Species= Speciescommonname)  #exclude when blue crabs weren't sampled
...
##There are no zero blue crab instances 
```

###### Overlap plots: 

```{r}
total_edt_new <- total_edt %>% filter(Speciescommonname %in% select_spp, !Area %in% "CAPEF", !Area %in% "NEWR", Year >= 2008, Longitude > -77.0, Month== 5|Month==6|Month==9)
...
P120_total_spp[[j]] <- P120_total %>% filter(Speciescommonname %in% j, Colnum > 0) %>% drop_na(Colnum)
P120_total_gridID[[j]] <- P120_total_spp[[j]] %>% dplyr::select(gridID, Season, Speciescommonname, Year) %>% group_by(Season, Year, Speciescommonname) %>% distinct(gridID, .keep_all= TRUE)
match_df[[j]] <- match_df(P915_total_gridID[[i]], P120_total_gridID[[j]], on = c("gridID", "Season", "Year")) 
}
}
...
for (i in spp_names) {
  ...
  P915_total_gridID[[i]] <- P915_total_spp[[i]] %>%
    dplyr::select(gridID, Season, Speciescommonname, Year) %>%
    group_by(Season, Year, Speciescommonname) %>%
  ...
  #  match_df[[i]][[j]] <- match_df(P915_total_gridID[[i]], #P120_total_gridID[[j]], on = c("gridID", "Season", "Year")) 
    match_df[[i]][[j]] <- inner_join(P915_total_gridID[[i]], P120_total_gridID[[j]], by = c("gridID", "Season", "Year")) %>%
