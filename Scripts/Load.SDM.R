...
##Binary dataset
df_binary <- read_csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_grid_binary_edt.11.22.23.csv")
...
test <- df_count %>% filter(Survey %in% "P120") %>% ungroup() %>% pivot_wider(names_from = "Speciescommonname", values_from = "avgcount")
##Binary 


test <- df_count %>% filter(Survey %in% "P120") %>% dplyr::select(-c(...1, CPUE, CPUE_stdzd, mean_CPUE, mean_CPUE_stdzd)) %>% pivot_wider(names_from = "Speciescommonname", values_from = "avg_count")
...
#IT'S P120 that is getting messed up!! 

CPUE_grid_avg_edt <- read.csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_grid_avg_edt.11.21.23.csv")
CPUE_grid_avg_edt <- CPUE_grid_avg_edt[,-1]
CPUE_grid_avg_edt <- CPUE_grid_avg_edt %>% mutate_at(c("Sedsize_common", "ShorelineType",  "Survey"), as.factor)
