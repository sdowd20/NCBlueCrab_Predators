...
coords_survey$lat_lon <- paste(coords_surveyedt$Latitude, coords_survey$Longitude, sep = "_") 
...
#Intersections2 drops some coordinates, let's figure out which ones, first shapefile retains all the points
...
coords2.2 <- st_coordinates(coords_surveysf)
coords_surveyedt <- coords_surveysf
coords_surveyedt$Latitude <- coords2.2[, "Y"]
coords_surveyedt$Longitude <- coords2.2[, "X"] #Add coordinates back in
coords_surveyedt$lat_lon <- paste(coords_surveyedt$Latitude, coords_surveyedt$Longitude, sep = "_") 
length(unique(coords_surveyedt$lat_lon)) #10,748
length(unique(predictors_df$lat_lon)) #9,068
column_diff <- setdiff(coords_surveyedt$lat_lon, predictors_df$lat_lon)

diff_coords <- coords_surveyedt %>% filter(lat_lon %in% column_diff)
intersections_diff <- st_intersection(diff_coords, NC_watersheds) 

ggplot(data= world) + geom_sf() + geom_point(data= intersections_diff %>% filter(HU_10_NAME %in% "New River Inlet"), aes(x= Longitude, y= Latitude, color= HU_10_NAME)) + theme(panel.background = element_rect(fill = "white", colour = "black")) + labs(x= "Longitude", y= "Latitude") + coord_sf(xlim=c(-78, -75), ylim=c(33,37), expand = TRUE)

unique(intersections2_edt$Wbd) #Names pungo Pamlico river (guess b/c main river)

intersections_diff_edt <- intersections_diff %>% mutate(Wbd = ifelse(HU_10_NAME %in% "Brunswick River-Cape Fear River"|HU_10_NAME %in% "Goshen Swamp"|HU_10_NAME %in% "Liliput Creek-Cape Fear River"|HU_10_NAME %in% "Harrisons Creek-Northeast Cape Fear River", "Cape Fear River", ifelse(HU_10_NAME %in% "South Creek-Pamlico River"|HU_10_NAME %in% "Chocowinity Bay-Pamlico River", "Pamlico River", ifelse(HU_10_NAME %in% "Pantego Creek"|HU_10_NAME %in% "Headwaters Pungo River"| HU_10_NAME %in% "Intracoastal Waterway-Pungo River"| HU_10_NAME %in% "Outlet Pungo River", "Pamlico River", ifelse(HU_10_NAME %in% "Pamlico Sound-Ocracoke Inlet"|HU_10_NAME %in% "Pamlico Sound-Hatteras Inlet"|HU_10_NAME %in% "Neuse River-Pamlico Sound"|HU_10_NAME %in% "Lake Mattamuskeet-Rose Bay"|HU_10_NAME %in% "Jones Bay-Bay River", "Pamlico Sound", ifelse(HU_10_NAME %in% "Headwaters New River"|HU_10_NAME %in% "New River Inlet", "New River", ifelse(HU_10_NAME %in% "Upper Broad Creek-Neuse River"|HU_10_NAME %in% "Cherry Point-Neuse River"|HU_10_NAME %in% "Town of Oriental-Neuse River", "Neuse River", ifelse(HU_10_NAME %in% "Oyster Creek-Jarrett Bay"|HU_10_NAME %in% "Cape Lookout Shoals-Core Banks", "Core Sound", ifelse(HU_10_NAME %in% "Bogue Banks-Shackleford Banks", "Bogue and Back Sound", ifelse(HU_10_NAME %in% "Albemarle Sound-Croatan Sound", "Croatan Sound", intersections_diff$HU_10_NAME)))))))))) %>% filter(!HU_10_NAME %in% c("Outlet Cashie River", "Shallotte River", "Tar River Reservoir-Tar River"))
unique(intersections_diff$HU_10_NAME)
unique(intersections_diff_edt$Wbd)
unique(intersections2_edt$Wbd)

#Removed Outlet Cashie River, Shallotte River, Tar River Reservoir-Tar River 
#In main dataset, bogue and back go to Bogue and Back Sound
#In this dataset, "Albemarle Sound-Croatan Sound" goes to Croatan Sound (eyed it on map)
#New River Inlet--> Stump

###Finish Wbd: bind datasets together 
unique(intersections2_edt$Wbd) #Hmm back sound isn't in here but still change it 
intersections2_edt <- intersections2 %>% mutate(Wbd= ifelse(waterbody %in% "Pamlico River And Pamlico Sound", "Pamlico Sound", ifelse(waterbody %in% "Bogue Sound (Incl. Intracoastal Wa", "Bogue and Back Sound", ifelse(waterbody %in% "Northeast Cape Fear River", "Cape Fear River", intersections2$waterbody)))) %>% select(Wbd, geometry, dist_inlets)
intersections_diff_edt <- intersections_diff_edt %>% select(Wbd, geometry, dist_inlets)
intersections_rbind <- rbind(intersections2_edt, intersections_diff_edt)

unique(intersections_rbind$Wbd)
##Wbd type: River to River, Sound, Inlet and Beach to Non-River
predictors_df <- intersections_rbind %>% mutate(Wbd_type= ifelse(grepl("River", Wbd), "River", ifelse(grepl("Sound", Wbd)|grepl("Beach", Wbd)|grepl("Inlet", Wbd), "Sound", predictors_df$Wbd)))
...
t <- CPUE_dfs_gridded_final %>% filter(Survey %in% "P915", Speciescommonname %in% "striped bass", Year== 2008, gridID== 46)
#2008, 46
