...
p915_CPUEedt <- p915_CPUE %>% select(-...1, -Obs, -Weather, -Winddir, -Windspd, -Sedsize, -Btmcomp, -doy, -Wbdytype, -Wbd, -Sample, -Secchi) %>% filter(Speciescommonname %in% c("atlantic croaker", "black drum", "red drum", "southern flounder", "southern kingfish", "striped bass", "bonnethead hammerhead", "sheepshead")) %>% filter(Year >= 2008) %>% rename("Location" = "Area", "Strata/station"= "Strata", "Depthzone"= "Quad") #added in species of focus for now (still waiting on data)
p120_CPUEedt <- p120_CPUE %>% filter(Core== 1|Core==2) %>% select(-...1, -Program, -Nbrrec3, -Time, -Duration, -doy, -Core, -Sedsize, -Btmcomp, -Secchi) %>% filter(Year >= 2008) %>% rename("Strata/station"= "Station") %>% mutate(Depthzone= "Constant") #still don't have btmcomp or sedsize 
...
p915_CPUEedt <- p915_CPUEedt %>% drop_na() #drops 6699 data points (8%)
p915_CPUEedt$Survey <- "P915"
p120_CPUEedt <- p120_CPUEedt %>% drop_na() #drops 871 data points (3%)
p120_CPUEedt$Survey <- "P120"
p195_CPUEedt <- p195_CPUEedt %>% drop_na() #drops 1650 data points (13%)
...
lat_lon_combo2 <- rbind(p915_CPUEedt %>% select(Latitude, Longitude, Survey), p120_CPUEedt %>% select(Latitude, Longitude, Survey), p195_CPUEedt %>% select(Latitude, Longitude, Survey))
...
globe_bb <- matrix(c(-78.900147,33.802938,
                      ...
                      -75.263672,36.222128,
                     -75.263672, 33.802938,
                     -78.900147,33.802938), byrow = TRUE, ncol = 2) %>% list() %>% st_polygon() %>% st_sfc(., crs = 4326) #08/13/23: moved latitude down a bit to help w/ grid cells 
...
#write.csv(fish_extract_2_2edt, "~/Documents/GitHub/NCBlueCrab_Predators/Data/SDM.grids.csv") #updated on 08/09/23
```
...
##Distance to SAV: go to QGIS with coords_survey_tojoin! Will need to redo this: just did to SAV in 2019/2020 to check importance
coords_survey_SAV <- read.csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/Predictor_variables/coords_survey_tojoin_qgis.csv")

##Join predictors, gridded, and CPUE datasets
###Predictors with gridID
predictors_df <- coords_survey_SAV %>% select(lat_lon, dist_inlets, HubDist) %>% rename("SAV_distkm"= "HubDist") %>% distinct(lat_lon, .keep_all = TRUE)
gridded_coords <- read.csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/SDM.grids.csv")
predictors_gridded <- predictors_df %>% left_join(gridded_coords, by= "lat_lon") %>% select(lat_lon, gridID, dist_inlets,SAV_distkm) #only 5 NA for gridID
...
pred_gridd_geom <- predictors_gridded1 %>% left_join(globe_grid_30 %>% rename("gridID"= "ID"), by= "gridID")
