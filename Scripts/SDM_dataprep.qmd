...
#Intersections2 drops some coordinates, let's figure out which ones 
...
coords_surveysf$lat_lon <- paste(coords_surveysf$Latitude, coords_surveysf$Longitude, sep= "_")
...
diff_coords <- coords_surveysf %>% filter(lat_lon %in% column_diff)

intersections_diff <- st_intersection(diff_coords, NC_watersheds) 

ggplot(data= world) + geom_sf() + geom_point(data= intersections_diff, aes(x= Longitude, y= Latitude, color= HU_10_NAME)) + coord_sf(xlim=c(-78, -75), ylim=c(33,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black")) + labs(x= "Longitude", y= "Latitude") #it works! just need to do it with less data
...
predictors_gridded <- predictors_df_edt %>% left_join(gridded_coords, by= "lat_lon") %>% select(lat_lon, gridID, dist_inlets)
