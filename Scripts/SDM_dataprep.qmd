...
p915_CPUEedt <- p915_CPUE %>% select(-...1, -Obs, -Weather, -Weather, -Sedsize, -Btmcomp, -doy, -Wbdytype, -Wbd, -Sample, -Ph, -Quad) %>% filter(Year >= 2001) %>% rename("Location" = "Area", "Strata/station"= "Strata") #added in species of focus for now (still waiting on data)
p120_CPUEedt <- p120_CPUE %>% filter(Core== 1|Core==2) %>% select(-...1, -Program, -Nbrrec3, -Time, -Duration, -doy, -Core, -Sedsize, -Btmcomp) %>% filter(Year >= 2001) %>% rename("Strata/station"= "Station")
p195_CPUEedt <- p195_CPUE %>% select(-...1, -Timestart, -Duration, -Depthend, -Wbdytype, -Wbd, -Speciesscientificname, -Depthzone) %>% rename("Ssal" = "Salinitysurface", "Bsal"= "Salinitybottom", "Stemp"= "Tempsurface", "Btemp"= "Tempbottom", "Latitude"= "Latitudestart", "Longitude"= "Longitudestart", "Colnum"= "Numbertotal", "Control1"= "Eventname", "Strata/station"= "Stationcode", "Depth"= "Depthstart", "Secchi"= "Secchi_depthcm") %>% filter(Year >= 2001)
...
Centroids4 <- Centroids4 %>% rename("ShorelineType"= "ShorelineT", "InletDist_km"= "InletDist_", "ShorelineDist_km"= "ShorelineD") %>% select(gridID, ShorelineType, InletDist_km, SAVDist_km)

selected_grids <- gridded_coords_geom_edt$gridID 
  
Grid_predictors <- Overall_grid9 %>% left_join(Centroids4, by= "gridID") %>% rename("gridArea_km"= "area_km2","ShrimpTrawl_p"= "ShrimpT_p", "TrawlDredge_p" ="TrawlDredg", "GillPoundNet_p"= "GillPound_", "WaterbodyCover"= "WaterbodyC") %>% filter(gridID %in% selected_grids) %>% select(-Waterbody)
...
```{r}
#Combining datasets
predictors_df <- read_csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/gridded_predictors_final.csv")
...
write.csv(CPUE_all_grid_pred,"~/Desktop/CPUE_all_grid_pred.csv") #updated on 08/23/23
```
...
```{r}
CPUE_dfs_gridded_final <- read_csv("~/Desktop/CPUE_all_grid_pred.csv")



CPUE_all_grid_pred  %>% filter(Speciescommonname %in% "blue crab", Year== 2008, Month== 06)
...
CPUE_grid<- CPUE_grid %>% group_by(gridID, Survey, Year) %>% mutate(mean_CPUE= mean(CPUE), mean_CPUE_stdzd= mean(CPUE_stdzd))


abund_bc <- CPUE_grid %>% filter(Speciescommonname %in% "blue crab") %>% group_by(gridID, Survey, Year) %>% mutate(mean_CPUE = mean(CPUE), mean_CPUE_stdzd= mean(CPUE_stdzd)) %>% distinct(mean_CPUE, .keep_all= TRUE)
...
p915_nums <- p915_CPUEedt %>% filter(Year > 2008,Month==6, Speciescommonname %in% interest_spp) %>% group_by(Speciescommonname, Month) %>% drop_na(Colnum) %>% summarise(Sum_colnum= sum(Colnum)) %>% mutate(Survey= "P915")
...
test <- CPUE_grid %>% filter(Year == 2008, Speciescommonname %in% "blue crab", Month==6)


ggplot() + geom_sf(data = world) + geom_point(data = subset(test, test$Survey %in% "P195" & test$Speciescommonname %in% "blue crab" & Colnum > 0), aes(x = Longitude, y = Latitude, size= mean_CPUE_stdzd), colour= "yellow") + scale_size_area() + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black"))
