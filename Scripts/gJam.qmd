...
xdata <- df_count_wide %>% dplyr::select(avg_depth, avg_stemp, avg_ssal, avg_sdo, SAVDist_km, InletDist_km, NoFishRest,  FishingAll_num, Sedsize_common, alewife:white.shrimp, -(c(red.drum, bonnethead.shark, black.drum, cownose.ray, southern.kingfish, striped.bass))) %>% mutate_at(vars(Sedsize_common), as.factor)
colnames(xdata) <- gsub(pattern = "_", replacement = ".", colnames(xdata))

colnames(df_count_wide)
##ydata: species of interest, same dimensions as xdata 
ydata <- df_count_wide %>% dplyr::select(red.drum, bonnethead.shark, black.drum, cownose.ray, southern.kingfish, striped.bass) 
...
formlst <- '~ avg.depth + avg.stemp + avg.ssal + avg.sdo + SAVDist.km + InletDist.km + NoFishRest + atlantic.menhaden + atlantic.croaker + southern.flounder + FishingAll.num + Sedsize.common'

out1 <- gjam(as.formula(formlst), 
                 xdata = xdata, 
                 ydata = ydata, 
                 modelList = ml)
...
xdata <- df_count_wide_na %>% dplyr::select(avg_depth, avg_stemp, avg_ssal, avg_sdo, SAVDist_km, InletDist_km, NoFishRest,  FishingAll_num, Sedsize_common, atlantic.menhaden.P915, atlantic.menhaden.P120, spot.P915, atlantic.croaker.P120, brown.shrimp.P120, blue.crab.P120, weakfish.P120) %>% mutate_at(vars(Sedsize_common), as.factor)
colnames(xdata) <- gsub(pattern = "_", replacement = ".", colnames(xdata))

##ydata: species of interest, same dimensions as xdata 
ydata <- df_count_wide_na %>% dplyr::select(red.drum.P915, bonnethead.shark.P915, black.drum.P915, cownose.ray.P915, southern.kingfish.P915, striped.bass.P915) 
