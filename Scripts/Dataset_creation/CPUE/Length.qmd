...
combo2_edt <- combo2 %>% mutate(Speciescommonname = ifelse(Age_class == "J", paste("juvenile", Speciescommonname), paste("adult", Speciescommonname))) %>% dplyr::select(-c(Colnum, Age_class)) %>% rename("Colnum"= "Num_sum")
...
P120_os_prop_edt <- P120_os_prop_edt %>% mutate(Speciescommonname = ifelse(Age_class == "J", paste("juvenile", Speciescommonname), paste("adult", Speciescommonname))) 
...
length(unique(P120_os_prop_edt$Speciescommonname)) #12
...
haul$Species=rep_along(haul$Species, c("bay anchovy", "brown shrimp", "pink shrimp", "white shrimp", "pinfish", "silver perch", "striped anchovy", "weakfish", "juvenile atlantic menhaden", "adult atlantic menhaden", "juvenile atlantic croaker", "adult atlantic croaker", "juvenile blue crab", "adult blue crab", "juvenile southern flounder", "adult southern flounder", "juvenile spot", "adult spot", "juvenile spotted seatrout", "adult spotted seatrout")) #this repeats these species common names for the entirety of the length of the haul df
...
length(unique(haul_final$Species)) #20 species
length(unique(haul_final$Control1)) #7053 Control1, 20*7053= 141060 or # of rows 
...
P120_os_prop_edt_tojoin <- P120_os_prop_edt %>% ungroup() %>% dplyr::select(Control1, Speciescommonname, Number_new) %>% rename("Colnum"= "Number_new")
...
haul_sel_spp <- left_join(haul_final, sel_spp_120joinedt, by= c("Speciescommonname", "Control1")) %>% as.data.frame() #df is 56,424 rows, 7053 rows for each species (8 species)
haul_sel_spp$Colnum[haul_sel_spp$Colnum %in% NA] <- 0 #If there is an NA this is because there was no data there 

names_join <- P120_bioledt %>% dplyr::select(Speciescommonname, Sciname) %>% distinct(Speciescommonname, .keep_all= T)
...
length(unique(p120_CPUE$Speciescommonname))
length(unique(p120_CPUE$Control1)) #it worked!!! redu the scientific name! 
```


```{r}
combo2_edt %>% filter(Speciescommonname %in% "juvenile atlantic menhaden") %>% ggplot() + geom_histogram(aes(x= Colnum)) + standard_theme + xlim(0,1000) + ylim(0,1000)

p120_CPUE %>% filter(Speciescommonname %in% "juvenile atlantic menhaden") %>% ggplot() + geom_histogram(aes(x= Colnum)) + standard_theme + xlim(0,1000) + ylim(0,1000)
```
