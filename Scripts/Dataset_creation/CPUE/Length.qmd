...
P120_os_prop_edt2 <- P120_os_prop_edt %>% mutate(Speciescommonname = ifelse(Age_class == "J", paste("juvenile", Speciescommonname), paste("adult", Speciescommonname))) 
...
haul$Species=rep_along(haul$Species, c("bay anchovy", "brown shrimp", "pink shrimp", "white shrimp", "pinfish", "silver perch", "striped anchovy", "weakfish", "juvenile atlantic menhaden", "adult atlantic menhaden", "juvenile atlantic croaker", "adult atlantic croaker", "juvenile blue crab", "adult blue crab", "juvenile southern flounder", "adult southern flounder", "juvenile spot", "adult spot", "juvenile spotted seatrout", "adult spotted seatrout", "spot")) #this repeats these species common names for the entirety of the length of the haul df
...
P120_os_prop_edt_tojoin <- P120_os_prop_edt2 %>% ungroup() %>% dplyr::select(Control1, Speciescommonname, Number_new) %>% rename("Colnum"= "Number_new") %>% filter(!Speciescommonname %in% c("adult spot", "juvenile spot")) #remove spot as had 9% error rate and all same size 
