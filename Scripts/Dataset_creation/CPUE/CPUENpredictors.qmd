...
p915_CPUEedt <- p915_CPUE %>% dplyr::select(-...1, -Obs, -Weather, -Weather, -Sedsize, -Btmcomp, -doy, -Wbdytype, -Wbd, -Sample, -Ph, -Quad, -Secchi) %>% rename("Location" = "Area", "Strata/station"= "Strata") %>% filter(between(Year, 2001, 2021), Month== c(5,6,9)) #added in species of focus for now (still waiting on data)
...
#1208 rows with NA for gridID, compared with dataset where I did not remove gridIDs outside study area (other than 543) and it was 39, keep this dataset 
...
#write.csv(CPUE_all_grid_pred,"~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_all_grid_pred.11.21.23.csv")
```
...
##depth: outliers range from 5.3 to 8.8 m 
t[4] 
min(CPUE_all_grid_pred$Depth, na.rm= TRUE) #0.1 m 
max(CPUE_all_grid_pred$Depth, na.rm= TRUE) #8.8 m
CPUE_all_grid_pred %>% filter(Depth >= 5.3) 
...
##Stemp: 88 extreme outliers in May and June, remove these 
t[7]
min(CPUE_all_grid_pred$Stemp, na.rm= TRUE) #0 
max(CPUE_all_grid_pred$Stemp, na.rm= TRUE) #35.8C
min(t[[7]]$Stemp) #0
max(t[[7]]$Stemp) #9.3 
subset(CPUE_all_grid_pred, Stemp <= 9.3) #all outliers in May and June
...
max(CPUE_all_grid_pred$SAV_km, na.rm= TRUE) #45.4
...
max(CPUE_all_grid_pred$Colnum, na.rm= TRUE) #8323
min(t[[11]]$Colnum) #5
max(t[[11]]$Colnum) #8323
...
ggplot() + geom_histogram(data= subset(CPUE_all_grid_pred, Colnum > 100), aes(x= Colnum)) + standard_theme +
facet_wrap(~Speciescommonname) + xlim(0, 1000) + ylim(0,250)
```
...
```{r}
#Form CPUE dataset 
CPUE_dfs_gridded_final <- read_csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_all_grid_pred.11.21.23.csv")
...
write.csv(CPUE_grid_avg_edt, "~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_grid_avg_edt.11.21.23.csv")
```
...
CPUE_grid_count_avg <- CPUE_grid_count %>% ungroup() %>% mutate(Stemp= ifelse(CPUE_grid_count$Stemp <= 9.3, NA, CPUE_grid_count$Stemp), Ssal= ifelse(CPUE_grid_count$Ssal==0.0, NA, CPUE_grid_count$Ssal), Sdo= ifelse(CPUE_grid_count$Sdo==0.0, NA, CPUE_grid_count$Sdo)) %>% group_by(Year, gridID, Month) %>% mutate(avg_depth= mean(Depth, na.rm= TRUE), avg_ssal= mean(Ssal, na.rm= TRUE), avg_bsal= mean(Bsal, na.rm= TRUE), avg_stemp= mean(Stemp, na.rm= TRUE), avg_btemp= mean(Btemp, na.rm= TRUE), avg_sdo= mean(Sdo, na.rm= TRUE), avg_bdo= mean(Bdo, na.rm= TRUE), avg_latitude= mean(Latitude, na.rm= TRUE), avg_longitude= mean(Longitude), na.rm= TRUE) %>% dplyr::select(-avg_bsal, -avg_btemp, -avg_bdo, -Stemp, -Btemp, -Sdo, -Bdo, -Ssal, -Bsal, -Season, -Sedsize_new, -Btmcomp_new, -Depth, -na.rm, -avg_latitude, -avg_longitude)
...
#write.csv(CPUE_grid_count_avg_edt,"~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_grid_count_avg_edt.11.21.23.csv")

t <- CPUE_grid_count_avg %>% filter(Survey %in% "P120") %>% dplyr::select(-c(CPUE, mean_CPUE, CPUE_stdzd, Colnum)) %>% ungroup() %>% pivot_wider(values_from = "avg_count", names_from = "Speciescommonname")
...
#write.csv(CPUE_grid_binary_edt,"~/Documents/GitHub/NCBlueCrab_Predators/Data/CPUE/CPUE_grid_binary_edt.11.22.23.csv") 
```
