...
p120_CPUE_length_multi <- read_csv("/Users/sallydowd/Documents/GitHub/NCBlueCrab_Predators/Data/P120/Finalized/p120_CPUE_length_multi.csv")
...
p915_CPUE_length_multiedt <- p915_CPUE_length_multi %>% dplyr::select(-...1, -Obs, -Weather, -Weather, -Sedsize, -Btmcomp, -doy, -Wbdytype, -Wbd, -Sample, -Ph, -Quad, -Secchi, -...2, -Photoperiod) %>% rename("Location" = "Area", "Strata/station"= "Strata") %>% filter(between(Year, 2001, 2021), Month %in% c(5,6,9)) 
...
p120_CPUE_length_multiedt <- p120_CPUE_length_multi %>% filter(Core== 1|Core==2) %>% dplyr::select(-...1, -Program, -Nbrrec3, -Time, -Duration, -doy, -Core, -Sedsize, -Btmcomp, -Secchi, -Photoperiod) %>% rename("Strata/station"= "Station") %>% filter(between(Year, 2001, 2021), Month %in% c(5,6))
p120_CPUE_BCedt <- p120_CPUE_BC %>% filter(Core== 1|Core==2) %>% dplyr::select(-X, -Program, -Nbrrec3, -Time, -Duration, -doy, -Core, -Sedsize, -Btmcomp, -Secchi, -Photoperiod) %>% rename("Strata/station"= "Station") %>% filter(between(Year, 2001, 2021), Month %in% c(5,6))
...
CPUE_length_grid_pred <- CPUE_length_grid_pred %>% drop_na(gridID) #drops 37 rows in each dataset CPUE_all_grid_pred_BC dataset
...
write.csv(CPUE_length_grid_pred, "~/Desktop/Ch1Data/CPUE/CPUE_all_grid_pred_multi.02.12.24.csv")
```
...
CPUE_grid_length_avg <- CPUE_grid_length_avg %>% mutate_all(~ifelse(is.nan(.), NA, .)) %>% drop_na() #drops 2426 rows
...
#Remove latitude, longitude, all fishing variables except NofishRest and FishingAll_num 
CPUE_grid_length_avg_edt <- CPUE_grid_length_avg %>% dplyr::select(-Latitude, -Longitude, -DPA_aA:-RestrictedArea_pA, -ShrimpTrawl_aA:-GillPound_aA, -lat_lon, -Day, -Location, -Colnum, -Date, -Ym_date, -Sciname, -"Strata.station", -Waterbody_A, -Effort, -Effort, -CPUE, -Month, -Control1)
...
write.csv(CPUE_grid_length_avg_edt, "~/Desktop/Ch1Data/CPUE/CPUE_grid_avg_lengthedt.02.12.24.csv")
```
