...
CPUE_all_grid_pred <- CPUE_all %>% left_join(gridpred_lat_lon, by= "lat_lon") %>% dplyr::select(lat_lon, everything()) #1208 rows with NA for gridID
#compared with dataset where I did not remove gridIDs outside study area
CPUE_all_grid_pred %>% group_by(gridID, Survey) %>% summarize(count= n())

summary(is.na(CPUE_all_grid_pred))
#143 rows with NA for gridID, 7 latitude and longitude points, all P915
