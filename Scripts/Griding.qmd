...
```{r}
fish_extract_comboedt <- read_csv("~/Documents/GitHub/NCBlueCrab_Predators/Data/gridded_10.02.23.5km.csv")

fish_extract_comboedt <- fish_extract_comboedt %>% separate(lat_lon, into = c('lat', 'lon'), sep="_")

fish_extract_geom <- fish_extract_comboedt %>% left_join(globe_grid_30 %>% rename("gridID"= "ID"), by= "gridID") #14239 vs. 14246, a couple points outside of gridded area 

nrow(fish_extract_geom)
nrow(lat_lon_combo)
length(unique(fish_extract_geom$gridID))
length(unique(fish_extract_comboedt$gridID)) #same amount of grid IDs
```
