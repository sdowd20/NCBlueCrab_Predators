...
t <- df_count %>% filter(Survey %in% "P915", Speciescommonname %in% c("bonnetheadshark", "southernkingfish", "reddrum", "blackdrum", "spot", "atlanticcroaker", "atlanticmenhaden", "southernkingfishforage"), avgcount > 0) %>% group_by(Speciescommonname, Year) %>% mutate(count= n()) %>% distinct(count)
...
tw_red_drum <- gam(reddrum~s(avgdepth)+s(avgssal)+s(avgsdo)+s(InletDistkm)+s(reddrum)+factor(FishingAllnum), data= df_count_wide_P915_train, family= "nb")
summary(tw_red_drum)
gam.check(tw_red_drum)

"reddrum~s(avgdepth)+s(avgstemp)+s(avgsdo)+s(SAVkm)+s(NoFishRest)+s(atlanticmenhaden)+s(atlanticcroaker)+s(southernflounder)+s(spot)+s(blackdrum)+s(pinfish)+s(reddrumforage)+factor(FishingAllnum)"
