...
res_rf <- tflow %>% fit()
  ...
  # delta_gam_pred <- predict(delta_gam, test, type = "link") %>% as.data.frame() %>% transmute(family = "Zero-inflated Poisson", pred = inv_link(V2) * exp(V1)) %>% mutate(red_drum= test$red_drum) %>% dplyr::select(pred)
  res_rf_pred <- res_rf %>% predict_training()
  ...
  # zinb$V4 <- AIC(zinb_mod)
  # zinb$V5 <- 1-pchisq(zinb_mod$deviance,  zinb_mod$df.residual)
  ...
  # nb_gam_df$V4 <- AIC(nb_gam)
  # nb_gam_df$V5 <- 1-pchisq(nb_gam$deviance,  nb_gam$df.residual)
  ...
  # pois_gam_df$V4 <- AIC(pois_gam)
   # pois_gam_df$V5 <- 1-pchisq(pois_gam$deviance,  pois_gam$df.residual)
  ...
  # tweedie_gam_df$V4 <- AIC(tweedie_gam)
  # tweedie_gam_df$V5 <- 1-pchisq(tweedie_gam$deviance,  tweedie_gam$df.residual)
 ...
 # delta_gam_df$V3 <- RMSE_func(delta_gam_pred$pred, test$red_drum)
  ...
  dat <- rbind(pois, zinb, nb_gam_df, pois_gam_df, tweedie_gam_df, res_rf_pred_df)
...
table <- compare_var %>% pivot_longer(R2:RMSE, names_to = "metric", values_to = "values") %>% group_by(model_type, metric) %>% summarise(mean_val = mean(values)) %>% pivot_wider(names_from = "metric", values_from = "mean_val") %>% arrange(-R2)
```
Add in delta model
Add in AIC and deviance explained 
