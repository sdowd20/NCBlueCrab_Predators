...
#Sest up loop 
set.seed(321)
compare_var_5km <- as.data.frame(matrix(ncol = 3, nrow = 0))
colnames(compare_var_5km) <- c("model_type", "R2", "RMSE")

compare_var_in_5km <- as.data.frame(matrix(ncol = 3, nrow = 0))
colnames(compare_var_in_5km) <- c("model_type", "R2", "RMSE")

Bootstrap_times_5km <- 100 #this is how many times i want the loop to run
smp_size_5km <- floor(0.70 * nrow(df_count_wide_5km)) #this is how i want to split training and testing data 
```

```{r}
for(i in 1:Bootstrap_times_5km) {
  train_ind <- sample(seq_len(nrow(df_count_wide_5km)), size = smp_size_5km)
  train <- df_count_wide_5km[train_ind, ]
  test <- df_count_wide_5km[-train_ind, ]
...
##Out of sample prediction 
 #GLMs and GAMs
  ...
  ##GLM, GAMs
    ...
    zinb$V3 <- RMSE_func(zinb_pred, test$red_drum)
    nb_gam_df <- as.data.frame(matrix(ncol =3))
    nb_gam_df$V1 <- "nb gam 5km"
    nb_gam_df$V2 <- r2_func(nb_gam_pred, test$red_drum)
    nb_gam_df$V3 <- RMSE_func(nb_gam_pred, test$red_drum)
    pois_gam_df <- as.data.frame(matrix(ncol = 3))
    pois_gam_df$V1 <- "pois gam 5km"
    pois_gam_df$V2 <- r2_func(pois_gam_pred, test$red_drum)
    pois_gam_df$V3 <- RMSE_func(pois_gam_pred, test$red_drum)
    tweedie_gam_df <- as.data.frame(matrix(ncol= 3))
    tweedie_gam_df$V1 <- "tweedie gam 5km"
    tweedie_gam_df$V2 <- r2_func(tweedie_gam_pred, test$red_drum)
    tweedie_gam_df$V3 <- RMSE_func(tweedie_gam_pred, test$red_drum)
    delta_gam_df <- as.data.frame(matrix(ncol = 3))
    delta_gam_df$V1 <- "delta gam 5km"
    delta_gam_df$V2 <- r2_func(delta_gam_pred$pred, test$red_drum)
    delta_gam_df$V3 <- RMSE_func(delta_gam_pred$pred, test$red_drum)

    ##Machine learning
    res_rf_pred_df <- as.data.frame(matrix(ncol= 3))
    res_rf_pred_df$V1 <- "rf 5km"
    res_rf_pred_df$V2 <- r2_func(res_rf_pred$.pred, test$red_drum)
    res_rf_pred_df$V3 <- RMSE_func(res_rf_pred$.pred, test$red_drum)
    res_bag_pred_df <- as.data.frame(matrix(ncol= 3))
    res_bag_pred_df$V1 <- "bagged 5km"
    res_bag_pred_df$V2 <- r2_func(res_bag_pred$.pred, test$red_drum)
    res_bag_pred_df$V3 <- RMSE_func(res_bag_pred$.pred, test$red_drum)
    res_boost_pred_df <- as.data.frame(matrix(ncol= 3))
    res_boost_pred_df$V1 <- "boosted 5km"
    res_boost_pred_df$V2 <- r2_func(res_bag_pred$.pred, test$red_drum)
    res_boost_pred_df$V3 <- RMSE_func(res_bag_pred$.pred, test$red_drum)
    
  dat_5km <- rbind(pois, zinb, nb_gam_df, pois_gam_df, tweedie_gam_df, delta_gam_df, res_rf_pred_df, res_bag_pred_df, res_boost_pred_df)
colnames(dat_5km) <- c("model_type", "R2", "RMSE")
  compare_var_5km <- rbind(compare_var_5km, dat_5km)
...
##In-sample:
  ...
  res_boost_pred_in <- res_boost %>% predict_training()
    ...
    ##GLM, GAMs
    pois_in <- as.data.frame(matrix(ncol= 3))
    pois_in$V1 <- "pois in 5km"
    pois_in$V2 <- r2_func(pois_pred_in, train$red_drum)
    pois_in$V3 <- RMSE_func(pois_pred_in, train$red_drum)
    zinb_in <- as.data.frame(matrix(ncol= 3))
    zinb_in$V1 <- "zinb in 5km"
    zinb_in$V2 <- r2_func(zinb_pred_in, train$red_drum)
    zinb_in$V3 <- RMSE_func(zinb_pred_in, train$red_drum)
    nb_gam_df_in <- as.data.frame(matrix(ncol= 3))
    nb_gam_df_in$V1 <- "nb gam in 5km"
    nb_gam_df_in$V2 <- r2_func(nb_gam_pred_in, train$red_drum)
    nb_gam_df_in$V3 <- RMSE_func(nb_gam_pred_in, train$red_drum)
    pois_gam_df_in <- as.data.frame(matrix(ncol= 3))
    pois_gam_df_in$V1 <- "pois gam in 5km"
    pois_gam_df_in$V2 <- r2_func(pois_gam_pred_in, train$red_drum)
    pois_gam_df_in$V3 <- RMSE_func(pois_gam_pred_in, train$red_drum)
    tweedie_gam_df_in <- as.data.frame(matrix(ncol= 3))
    tweedie_gam_df_in$V1 <- "tweedie gam in 5km"
    tweedie_gam_df_in$V2 <- r2_func(tweedie_gam_pred_in, train$red_drum)
    tweedie_gam_df_in$V3 <- RMSE_func(tweedie_gam_pred_in, train$red_drum)
    delta_gam_df_in <- as.data.frame(matrix(ncol= 3))
    delta_gam_df_in$V1 <- "delta gam in 5km"
     delta_gam_df_in$V2 <- r2_func(delta_gam_pred_in$pred, train$red_drum)
    delta_gam_df_in$V3 <- RMSE_func(delta_gam_pred_in$pred, train$red_drum)
    res_boost_pred_df_in <- as.data.frame(matrix(ncol= 3))
    res_boost_pred_df_in$V1 <- "boost tree in 5km"
    res_boost_pred_df_in$V2 <- r2_func(res_bag_pred_in$.pred, train$red_drum)
    res_boost_pred_df_in$V3 <- RMSE_func(res_bag_pred_in$.pred, train$red_drum)
   res_rf_pred_df_in <- as.data.frame(matrix(ncol= 3))
   res_rf_pred_df_in$V1 <- "rf in 5km"
     res_rf_pred_df_in$V2 <- r2_func(res_rf_pred_in$.pred, train$red_drum)
    res_rf_pred_df_in$V3 <- RMSE_func(res_rf_pred_in$.pred, train$red_drum)
    res_bag_pred_df_in <- as.data.frame(matrix(ncol= 3))
    res_bag_pred_df_in$V1 <- "bag in 5km"
    res_bag_pred_df_in$V2 <- r2_func(res_bag_pred_in$.pred, train$red_drum)
    res_bag_pred_df_in$V3 <- RMSE_func(res_bag_pred_in$.pred, train$red_drum)
    
  dat_in_5km <- rbind(pois_in, zinb_in, nb_gam_df_in, pois_gam_df_in, tweedie_gam_df_in, delta_gam_df_in, res_rf_pred_df_in, res_bag_pred_df_in, res_boost_pred_df_in)
colnames(dat_in_5km) <- c("model_type", "R2", "RMSE")
  compare_var_in_5km <- rbind(compare_var_in_5km, dat_in_5km)
}

compare_out_5km <- compare_var_5km %>% pivot_longer(R2:RMSE, names_to = "metric", values_to = "values") %>% group_by(model_type, metric) %>% summarise(mean_val = mean(values)) %>% pivot_wider(names_from = "metric", values_from = "mean_val") %>% arrange(-R2)
compare_in_5km <- compare_var_in_5km %>% pivot_longer(R2:RMSE, names_to = "metric", values_to = "values") %>% group_by(model_type, metric) %>% summarise(mean_val = mean(values)) %>% pivot_wider(names_from = "metric", values_from = "mean_val") %>% arrange(-R2)
```


```{r}
#Calculate AIC and deviance explained separately for models
pois_mod_5km <- glm(model_formula, family="poisson", data=df_count_wide_5km)
  # nb_mod <- glm.nb(model_formula, data = train)
  # # zipois_mod <- zeroinfl(model_formula, dist = 'negbin', data = train)
  zinb_mod_5km <- zeroinfl(model_formula, dist = 'poisson', data = df_count_wide_5km)
  nb_gam_5km <- gam(gam_formula, family= "nb", data= df_count_wide_5km)
  pois_gam_5km <- gam(gam_formula, family=poisson(), data= df_count_wide_5km)
 tweedie_gam_5km <- gam(gam_formula, family= tw(link= "log"), method= "REML", data=df_count_wide_5km)
    delta_gam_5km <- gam(list(gam_formula, ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(atlantic_menhaden, bs="ts", k=5) + s(atlantic_croaker, bs="ts", k=5) + s(southern_flounder, bs="ts", k=5) + s(spot, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common)), family= ziplss(), data= df_count_wide_5km, method = "REML")
  
AIC <- AIC(pois_mod_5km, zinb_mod_5km, nb_gam_5km, pois_gam_5km, tweedie_gam_5km)
```

```{r}
#Model output
summary(pois_gam_5km)
gam.check(pois_gam_5km)
```

#### P915, p120, 5km
```{r}
df_count_5km$SpeciesSurvey <- paste(df_count_5km$Speciescommonname, df_count_5km$Survey, sep= "_")

df_count_wide_5km_na <- df_count_5km %>% filter(Survey %in% "P915"|Survey %in% "P120") %>% dplyr::select(-Speciescommonname, -Survey) %>% ungroup() %>% pivot_wider(names_from = "SpeciesSurvey", values_from = "avg_count") %>% drop_na()
#199 for P915, P120
#66 for P915, P195
```
...
#Sest up loop 
set.seed(321)
compare_var_5km_na <- as.data.frame(matrix(ncol = 3, nrow = 0))
colnames(compare_var_5km_na) <- c("model_type", "R2", "RMSE")

compare_var_in_5km_na  <- as.data.frame(matrix(ncol = 3, nrow = 0))
colnames(compare_var_in_5km_na ) <- c("model_type", "R2", "RMSE")

Bootstrap_times_5km_na  <- 100 #this is how many times i want the loop to run
smp_size_5km_na  <- floor(0.70 * nrow(df_count_wide_5km_na)) #this is how i want to split training and testing data 
...
```{r}
for(i in 1:Bootstrap_times_5km_na) {
  train_ind <- sample(seq_len(nrow(df_count_wide_5km_na)), size = smp_size_5km_na)
  train <- df_count_wide_5km_na[train_ind, ]
  test <- df_count_wide_5km_na[-train_ind, ]

  #GLMs and GAMs
  # pois_mod <- glm(model_formula_na, family="poisson", data=train)
  # zinb_mod <- zeroinfl(model_formula_na, dist = 'poisson', data = train)
  # nb_gam <- gam(gam_formula_na, family= "nb", data= train)
  # pois_gam <- gam(gam_formula_na, family=poisson(), data= test), fewer unique covariates 
 # tweedie_gam <- gam(gam_formula_na, family= tw(link= "log"), method= "REML", data=train)
  # delta_gam <- gam(list(gam_formula_na, ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(atlantic_menhaden_P915, bs="ts", k=5) + s(atlantic_croaker_P915, bs="ts", k=5) + s(southern_flounder_P915, bs="ts", k=5) + s(spot_P915, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common) + s(blue_crab_P120, bs="ts", k=5) + s(weakfish_P120, bs="ts", k=5) + s(brown_shrimp_P120, bs="ts", k=5) + s(atlantic_menhaden_P120, bs="ts", k=5) + s(atlantic_croaker_P120, bs="ts", k=5) + s(pinfish_P120, bs="ts", k=5) + s(pink_shrimp_P120, bs="ts", k=5) + s(southern_flounder_P120, bs="ts", k=5) + s(spot_P120, bs="ts", k=5)), family= ziplss(), data= train, method = "REML")
...
##Out of sample prediction 
 #GLMs and GAMs
  ...
  ##GLM, GAMs
    ...
    # nb_gam_df$V3 <- RMSE_func(nb_gam_pred, test$red_drum_P915)
    # pois_gam_df <- as.data.frame(matrix(ncol = 3))
    # pois_gam_df$V1 <- "pois gam 5km na"
    # pois_gam_df$V2 <- r2_func(pois_gam_pred, test$red_drum_P915)
    # pois_gam_df$V3 <- RMSE_func(pois_gam_pred, test$red_drum_P915)
    # tweedie_gam_df <- as.data.frame(matrix(ncol= 3))
    # tweedie_gam_df$V1 <- "tweedie gam 5km na"
    # tweedie_gam_df$V2 <- r2_func(tweedie_gam_pred, test$red_drum_P915)
    # tweedie_gam_df$V3 <- RMSE_func(tweedie_gam_pred, test$red_drum_P915)
    # delta_gam_df <- as.data.frame(matrix(ncol = 3))
    # delta_gam_df$V1 <- "delta gam 5km na"
    # delta_gam_df$V2 <- r2_func(delta_gam_pred$pred, test$red_drum_P915)
    # delta_gam_df$V3 <- RMSE_func(delta_gam_pred$pred, test$red_drum_P915)

    ##Machine learning
    res_rf_pred_df <- as.data.frame(matrix(ncol= 3))
    res_rf_pred_df$V1 <- "rf 5km na"
    res_rf_pred_df$V2 <- r2_func(res_rf_pred$.pred, test$red_drum_P915)
    res_rf_pred_df$V3 <- RMSE_func(res_rf_pred$.pred, test$red_drum_P915)
    res_bag_pred_df <- as.data.frame(matrix(ncol= 3))
    res_bag_pred_df$V1 <- "bagged 5km na"
    res_bag_pred_df$V2 <- r2_func(res_bag_pred$.pred, test$red_drum_P915)
    res_bag_pred_df$V3 <- RMSE_func(res_bag_pred$.pred, test$red_drum_P915)
    res_boost_pred_df <- as.data.frame(matrix(ncol= 3))
    res_boost_pred_df$V1 <- "boosted 5km na"
    res_boost_pred_df$V2 <- r2_func(res_bag_pred$.pred, test$red_drum_P915)
    res_boost_pred_df$V3 <- RMSE_func(res_bag_pred$.pred, test$red_drum_P915)
    
  dat_5km_na <- rbind(res_rf_pred_df, res_bag_pred_df, res_boost_pred_df)
colnames(dat_5km_na) <- c("model_type", "R2", "RMSE")
  compare_var_5km_na <- rbind(compare_var_5km_na, dat_5km_na)
...
##In-sample:
  ...
  res_boost_pred_in <- res_boost %>% predict_training()
    ...
    # nb_gam_df_in$V3 <- RMSE_func(nb_gam_pred_in, train$red_drum_P915)
    # pois_gam_df_in <- as.data.frame(matrix(ncol= 3))
    # pois_gam_df_in$V1 <- "pois gam in 5km na"
    # pois_gam_df_in$V2 <- r2_func(pois_gam_pred_in, train$red_drum_P915)
    # pois_gam_df_in$V3 <- RMSE_func(pois_gam_pred_in, train$red_drum_P915)
    # tweedie_gam_df_in <- as.data.frame(matrix(ncol= 3))
    # tweedie_gam_df_in$V1 <- "tweedie gam in 5km na"
    # tweedie_gam_df_in$V2 <- r2_func(tweedie_gam_pred_in, train$red_drum_P915)
    # tweedie_gam_df_in$V3 <- RMSE_func(tweedie_gam_pred_in, train$red_drum_P915)
    # delta_gam_df_in <- as.data.frame(matrix(ncol= 3))
    # delta_gam_df_in$V1 <- "delta gam in 5km na"
    #  delta_gam_df_in$V2 <- r2_func(delta_gam_pred_in$pred, train$red_drum_P915)
    # delta_gam_df_in$V3 <- RMSE_func(delta_gam_pred_in$pred, train$red_drum_P915)
    res_boost_pred_df_in <- as.data.frame(matrix(ncol= 3))
    res_boost_pred_df_in$V1 <- "boost tree in 5km na"
    res_boost_pred_df_in$V2 <- r2_func(res_bag_pred_in$.pred, train$red_drum_P915)
    res_boost_pred_df_in$V3 <- RMSE_func(res_bag_pred_in$.pred, train$red_drum_P915)
   res_rf_pred_df_in <- as.data.frame(matrix(ncol= 3))
   res_rf_pred_df_in$V1 <- "rf in 5km na"
     res_rf_pred_df_in$V2 <- r2_func(res_rf_pred_in$.pred, train$red_drum_P915)
    res_rf_pred_df_in$V3 <- RMSE_func(res_rf_pred_in$.pred, train$red_drum_P915)
    res_bag_pred_df_in <- as.data.frame(matrix(ncol= 3))
    res_bag_pred_df_in$V1 <- "bag in 5km na"
    res_bag_pred_df_in$V2 <- r2_func(res_bag_pred_in$.pred, train$red_drum_P915)
    res_bag_pred_df_in$V3 <- RMSE_func(res_bag_pred_in$.pred, train$red_drum_P915)
    
  dat_in_5km_na <- rbind(res_rf_pred_df_in, res_bag_pred_df_in, res_boost_pred_df_in)
colnames(dat_in_5km_na) <- c("model_type", "R2", "RMSE")
  compare_var_in_5km_na <- rbind(compare_var_in_5km_na, dat_in_5km_na)
}

compare_out_5km_na <- compare_var_5km_na %>% pivot_longer(R2:RMSE, names_to = "metric", values_to = "values") %>% group_by(model_type, metric) %>% summarise(mean_val = mean(values)) %>% pivot_wider(names_from = "metric", values_from = "mean_val") %>% arrange(-R2)
compare_in_5km_na <- compare_var_in_5km_na %>% pivot_longer(R2:RMSE, names_to = "metric", values_to = "values") %>% group_by(model_type, metric) %>% summarise(mean_val = mean(values)) %>% pivot_wider(names_from = "metric", values_from = "mean_val") %>% arrange(-R2)
```


```{r}
#Calculate AIC and deviance explained separately for models
pois_mod_5km <- glm(model_formula_na, family="poisson", data=df_count_wide_5km)
  # nb_mod <- glm.nb(model_formula, data = train)
  # # zipois_mod <- zeroinfl(model_formula, dist = 'negbin', data = train)
  zinb_mod_5km <- zeroinfl(model_formula, dist = 'poisson', data = df_count_wide_5km)
  nb_gam_5km <- gam(gam_formula, family= "nb", data= df_count_wide_5km)
...
AIC <- AIC(pois_mod_5km, zinb_mod_5km, nb_gam_5km, pois_gam_5km, tweedie_gam_5km)
  ...
  res_rf_pred <- res_rf %>% predict_testing()
  res_rf_pred_train <- res_rf %>% predict_training()
```
