...
##In-sample:
  ...
  res_boost_pred_in <- res_boost %>% predict_training()
    ...
    ##GLM, GAMs
    pois_in <- as.data.frame(matrix(ncol= 2))
    pois_in$V1 <- r2_func(pois_pred_in, train$red_drum)
    pois_in$V2 <- RMSE_func(pois_pred_in, train$red_drum)
    zinb_in <- as.data.frame(matrix(ncol= 2))
    zinb_in$V1 <- r2_func(zinb_pred_in, train$red_drum)
    zinb_in$V2 <- RMSE_func(zinb_pred_in, train$red_drum)
    nb_gam_df_in <- as.data.frame(matrix(ncol= 2))
    nb_gam_df_in$V1 <- r2_func(nb_gam_pred_in, train$red_drum)
    nb_gam_df_in$V2 <- RMSE_func(nb_gam_pred_in, train$red_drum)
    pois_gam_df_in <- as.data.frame(matrix(ncol= 2))
    pois_gam_df_in$V1 <- r2_func(pois_gam_pred_in, train$red_drum)
    pois_gam_df_in$V2 <- RMSE_func(pois_gam_pred_in, train$red_drum)
    tweedie_gam_df_in <- as.data.frame(matrix(ncol= 2))
    tweedie_gam_df_in$V1 <- r2_func(tweedie_gam_pred_in, train$red_drum)
    tweedie_gam_df_in$V2 <- RMSE_func(tweedie_gam_pred_in, train$red_drum)
    delta_gam_df_in <- as.data.frame(matrix(ncol= 2))
     delta_gam_df_in$V1 <- r2_func(delta_gam_pred_in$pred, train$red_drum)
    delta_gam_df_in$V2 <- RMSE_func(delta_gam_pred_in$pred, train$red_drum)
    res_boost_pred_df_in <- as.data.frame(matrix(ncol= 2))
    res_boost_pred_df_in$V1 <- r2_func(res_bag_pred_in$.pred, train$red_drum)
    res_boost_pred_df_in$V2 <- RMSE_func(res_bag_pred_in$.pred, train$red_drum)
   res_rf_pred_df_in <- as.data.frame(matrix(ncol= 2))
     res_rf_pred_df_in$V1 <- r2_func(res_rf_pred_in$.pred, train$red_drum)
    res_rf_pred_df_in$V2 <- RMSE_func(res_rf_pred_in$.pred, train$red_drum)
    res_bag_pred_df_in <- as.data.frame(matrix(ncol= 2))
    res_bag_pred_df_in$V1 <- r2_func(res_bag_pred_in$.pred, train$red_drum)
    res_bag_pred_df_in$V2 <- RMSE_func(res_bag_pred_in$.pred, train$red_drum)
...
AIC(pois_mod, zinb_mod, nb_gam, pois_gam, tweedie_gam)
