...
##### Models
```{r}
southernkingfish_env <- gam(southernkingfishP915~s(avgdepth, k=11)+s(avgssal, k=11)+s(avgstemp, k=11)+s(avgsdo, k=11)+s(NoFishRest, k=11)+factor(Yearfactor), family= tw(), data=df_CPUE_length_wide_both)

southernkingfish_bc <- gam(southernkingfishP915~ s(log(smallbluecrabP120+1), k=22) + factor(Yearfactor), family= tw(), data=df_CPUE_length_wide_both)

southernkingfish_forage <- gam(southernkingfishP915~ s(log(southernkingfishP915forageP915+1), k= 66) + s(log(southernkingfishP915forageP120+1), k=66) + factor(Yearfactor), family= tw(), data=df_CPUE_length_wide_both)
...
#Error: algorithm can't take a step in parameter space that would result in an improvement in the optimizaiton criterion
...
cor(df_CPUE_length_wide_both %>% dplyr::select(avgdepth,avgssal, avgstemp, avgsdo, NoFishRest, blackdrumP915forageP915, blackdrumP915forageP120, smallbluecrabP120))
