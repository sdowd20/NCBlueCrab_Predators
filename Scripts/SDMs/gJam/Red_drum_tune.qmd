...
Changes on 12/03/23: 
-Forage for each predator should include blue crabs for gJam, changes made to script to load in, dataset now: df_count_wide_both_gJam
...
cor_sk <- modo_sk[["parameters"]][["corMu"]] #residual
resid_sk <- as.data.frame(cor_sk)
resid_sk$names <- rownames(cor_sk)
resid_sk <- resid_sk %>% dplyr::select(southernkingfishP915, names) %>% dplyr::filter(names != "southernkingfishP915")

#Start for loop 
for(i in 1:Bootstrap_times) {
#randomly select fish to use to predict
use_sk <- resid_sk %>% top_n(20, southernkingfishP915)
fish_sk <- sample(use_sk$names, 4, replace = F)
specs1_sk <- fish_sk
ynames_sk <- c(specs1_sk, "southernkingfishP915")
# xdata_insample <- df %>% dplyr::select(avgstemp, avgsdo, InletDistkm, NoFishRest)
...
r2_con_sk <- r2_general(dat_sk$pred_sk_con, dat_sk$obs__sk)
r2_unco_skn <- r2_general(dat_sk$pred_sk_un, dat_sk$obs_sk)
rmse_con_sk <-  RMSE_func(actual = dat_sk$obs_sk, pred = dat_sk$pred_sk_con)
rmse_uncon_sk <-  RMSE_func(actual = dat_sk$obs_sk, pred = dat_sk$pred_sk_un)

model_compare_sk[i,1] <- r2_con_sk
model_compare_sk[i,2] <- r2_uncon_sk
model_compare_sk[i,3] <- rmse_con_sk
model_compare_sk[i,4] <- rmse_uncon_sk

model_compare_sk[i,5] <- fish_sk[1]
model_compare_sk[i,6] <- fish_sk[2]
model_compare_sk[i,7] <- fish_sk[3]
model_compare_sk[i,8] <- fish_sk[4]
print(i)
} 
