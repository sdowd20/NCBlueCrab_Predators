...
```{r}
xdata <- df_count_wide_both %>% dplyr::select(avgdepth, avgstemp, avgssal, avgsdo, SAVkm, InletDistkm, NoFishRest, FishingAllnum, Sedsizecommon) %>% mutate_at(vars(c(Sedsizecommon, FishingAllnum)), as.factor)
ydata <- df_count_wide_both %>% dplyr::select(atlanticcroakerP915:bonnetheadsharkP915forage) 

smp_size <- floor(0.70 * nrow(df_count_wide_both)) #this is how i want to split training and testing data 

#Reduce # of species included 
species  <- gjamTrimY(ydata, 30, OTHER = FALSE)$y %>% colnames() #minimum # of non-zero observations: 30 
...
```{r}
ydata_reddrum <- ydata[, -c(26, 28:30)]

# #Original gJam model: 
# ml <- list(ng = 60000, burnin = 50000, typeNames = 'DA')
# 
# formula_str <- paste("~", paste(colnames(xdata)[c(2,4,6,7)], collapse = " + "))
# 
# modo <- gjam(formula= as.formula(formula_str), xdata = xdata, ydata = ydata_reddrum, modelList = ml)
# #only lets you run with 26 variables 
# 
# save(modo, file= "~/Desktop/Ch1Data/gJam/10km/Updated_results/Red_drum/Initial_models/modo1.RData")
```
...
ydata_insample <- df[, colnames(df) %in% ynames]

# ml <- list(ng = 60000, burnin = 50000, typeNames = 'DA')

ml <- list(ng = 600, burnin = 500, typeNames = 'DA')

mod2 <- gjam(formula=~ avgstemp + avgsdo + InletDistkm + NoFishRest, xdata = xdata, ydata = ydata_insample, modelList = ml)
...
newdata <- list(xdata = xdata_insample, nsim = 1000)
