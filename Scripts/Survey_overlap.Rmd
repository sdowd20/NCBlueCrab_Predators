...
total_edt <- total_edt %>% filter(Speciescommonname %in% select_spp)
...
ggplot() + geom_sf(data = world) + geom_point(data = subset(total_edt, Colnum > 0), aes(x = Longitude, y = Latitude)) + scale_size_area() + facet_wrap(~Survey) + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) + theme(panel.background = element_rect(fill = "white", colour = "black"))
```
...
for(i in spp_names) {
P915_total_spp[[i]] <- P915_total %>% filter(Speciescommonname %in% i)
...
spp_names <- unique(P195_total$Speciescommonname)
P915_total_spp <- list()
P915_total_gridID <- list()
P120_total_spp <- list()
P120_total_gridID <- list()
...
#temporal piece Figure out if the overlapping data has similar time frames
...
Carcharhinus.plumbeus = sandbarshark, 133 positive abundances Ginglymostoma.cirratum = nurse shark, 176 positive abundances Galeocerdo.cuvier = tiger shar, 74 positive abundances Rhizoprionodon.terraenovae = sharpnose shark, 427 positive abundances
