...
#For each species and each dataset, loop through to see the overlapping gridIDs with the other datasets
...
spp_names <- unique(df_new$Species)
P915_total_spp <- list()
...
SST_data <- as.data.frame(do.call(rbind, match_df))
SST_data %>% group_by(Species, Season) %>% dplyr::mutate(count= n()) %>% distinct(count, .keep_all= TRUE)

#double check this! create match_df for 
match_df(P915_total_gridID[[4]], P120_total_gridID[[4]])
match_df(P915_total_gridID[[7]], P120_total_gridID[[7]])
