...
total_edt <- total_edt %>% filter(Speciescommonname %in% select_spp, !Area %in% "CAPEF", !Area %in% "NEWR", Year >= 2008, Season %in% "Spring"| Season %in% "Summer", Month == 5| Month== 6| Month== 9, Longitude > -77.0)
...
P120_bc_sum <- P120_total %>% filter(Colnum > 0, Speciescommonname %in% "blue crab") %>% dplyr::group_by(Year, Season, gridID) %>% summarise(sum_Colnum_bc = sum(Colnum)) match_data2edt <- match_data2 %>% left_join(P915_total_sum) %>% left_join(P120_bc_sum, by= c("Year", "Season", "gridID"))

match_data2edt %>% rename(Species = Speciescommonname) %>% group_by(Species) %>% summarize(sum_spp = sum(sum_Colnum_pred), sum_bc = sum(sum_Colnum_bc))%>% gt() %>% cols_width(everything() ~ px(100)) %>% cols_align(align= "center") %>% gtsave(filename = "~/Desktop/pospos.png")
print(names(match_data2edt))
