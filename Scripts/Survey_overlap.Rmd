...
P915_total_spp[[i]] <- P915_total %>% filter(Speciescommonname %in% i, Colnum > 0)
P915_total_gridID[[i]] <- P915_total_spp[[i]] %>% dplyr::select(gridID, Season, Speciescommonname, Year) %>% group_by(Season, Year, Speciescommonname) %>% distinct(gridID, .keep_all= TRUE)
P120_bc <- P120_total %>% filter(Speciescommonname %in% "blue crab")
...
library(gghighlight)
ggplot(data = world) + geom_sf() + geom_sf(data = globe_grid_20, fill = NA) + geom_point(data = subset(total_edt, total_edt$Species %in% "blue catfish" & Colnum > 0), aes(x = Longitude, y = Latitude)) + gghighlight(gridID == 20) + scale_size_area() + facet_wrap(~Survey) + coord_sf(xlim=c(-78, -75), ylim=c(34,37), expand = TRUE) 
