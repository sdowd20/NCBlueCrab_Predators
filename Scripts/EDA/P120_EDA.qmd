...
haul <- P120_bioledt %>% group_by(Date, Location, Station) %>% mutate(ID=row_number()) %>% filter(ID==1) %>% as.data.frame() #row every haul: Dan suggested grouping by Date, Location and Station b/c location is waterbody at large and some stations have the same name (need Location)
length(unique(haul$Control1)) #9663
length(unique(P120_bioledt$Control1)) #9668
#Let's look at which values are different: after 2008, there is only 1 Control1 and station that is different (38 rows)
diff_values <- setdiff(P120_bioledt$Control1, haul$Control1)
P120_bioledt %>% filter(Control1 %in% diff_values)
