...
P120_bc_test <- P120_bc %>% select(Control1, Date, Location, Colnum, Samnum, Subnum, Linenum, Frequenc, Length, Speciescommonname) %>% drop_na(Colnum, Length) #63690 to 63680 when drop_na
 ...
 #Make sure each crab sampling has an individual row 
...
min(P120_bc$Length) #min is 1, max is 266 mm
max(P120_bc$Length)
...
P120_bc_prop$Age_class <- ifelse(P120_bc_prop$Assigned_length < 50, "J", "A") #assign juvenile if under 50 cm (harvestable length, after 1 year)
...
#Count how many of each age class, calculate how many were sampled for Control1 and Colnum, calculate proportion of each


t<- P120_bc_prop %>% filter(Control1 == "9712836")
colnames(P120_bc_edt)

#Assign as juvenile or adult
#50.8 mm (2 inches) is the harvestable length, blue crabs should reach this length in ~1 year when they become sexually mature

summary(P120_bc$Assigned_length)
#Proportion of each length bin for Colnum 
