...
result_df <- P120_bc_test %>% group_by(Control1, Colnum, Length) %>% uncount(Frequenc, .remove= FALSE) %>% mutate(Frequenc_new = 1) #Now Frequenc 
...
result_df_new <- result_df %>%
  ...
  mutate(
    rows_to_add = Colnum - n(),
    Length2 = if_else(rows_to_add > 0, mean(Length), Length)
  ) %>%
  ungroup() %>%
  mutate(
    Length = if_else(row_number() <= n(), Length2, Length)
  ) %>%
  select(-Length2)
...
result_df_new <- result_df %>%
  ...
  ungroup() %>%
  complete(Control1, Colnum, Length = Length2) %>%
  select(-Length2)


  
?slice()
result_df_new <- result_df_ %>%
  group_by(Control1, Colnum) %>%
  mutate(rows_to_add = Colnum - n()) %>%
  slice(1:n()) %>%
  mutate(Length = if_else(rows_to_add > 0, mean(Length), Length)) %>%
...
result_df_new <- result_df %>%
  group_by(Control1, Colnum) %>%
  mutate(
    rows_to_add = Colnum - n()) %>% slice(1:n()) %>% 
