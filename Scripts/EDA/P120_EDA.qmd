...
##Fix issue with location: same locations one just has a zero and other doesn't
P120_bioledt$Location <- ifelse(substr(P120_bioledt$Location, 1, 1) == "0", substr(P120_bioledt$Location, 2, nchar(P120_bioledt$Location)), P120_bioledt$Location) #substr: check if first integer in Location column = 0, if it does than extract substring at second character (remove leading 0), need nchar to determine length of each element, did this because there were repeating locations in Control1 that appeared to be same location (528410300 vs. 0528410300)
...
length(unique(P120_bioledt$Control1)) #full dataset has more unique control1 than haul, 9641 vs. 9330
haul <- P120_bioledt %>% group_by(Date, Location) %>% mutate(ID=row_number()) %>% filter(ID==1) %>% as.data.frame() #row every haul (9330 variables whereas Lela had 8099), for all species
length(unique(haul$Control1))
#Let's look at which values are different: there are ~20 not in main cores
...
haul_df_all <- haul_df %>% tidyr::expand(Species,Control1) ##using expand to make all three species associated with each collection number, is 9x (# of species) larger so it worked
...
sel_spp_1202= sel_spp_120%>%select(Control1,Date,Speciescommonname,Colnum)

haul_final <- haul_final %>% rename("Speciescommonname" = "Species")
haul_sel_spp =left_join(haul_final, sel_spp_1202)%>%as.data.frame()
