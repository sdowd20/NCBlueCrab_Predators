...
#50.8 cm (2 inches) is the harvestable length, blue crabs should reach this length in ~1 year when they become sexually mature
 ...
 #%>% mutate("Ageclass"= ifelse(Length < 50.8, "juvenile", ifelse(Length >= 50.8, "adult", "NA"))) 
...
P120_bc_test <- P120_bc %>% select(Control1, Date, Location, Colnum, Samnum, Subnum, Linenum, Frequenc, Length, Speciescommonname) %>% drop_na(Colnum, Length) #63690 to 63680 when drop_na
...
result_df <- P120_bc %>% group_by(Control1, Colnum, Length) %>% uncount(weights=Frequenc, .remove= FALSE) %>% mutate(Frequenc_new = 1) #Make sure each crab sampling has an individual row 
#uncount: opposite direction of count, weights (Frequenc) specifies the number of times a row should be repeated 
#Subnum: # of individuals sampling in unit 


result_df_new <- result_df %>%
  group_by(Control1, Colnum) %>%
  mutate(rows_to_add = Colnum - n()) %>%
  slice(1:n()) %>%
  mutate(Length2 = if_else(rows_to_add > 0, mean(Length), Length)) %>%
  slice(rep(1:n(), times = pmax(rows_to_add, 0) + 1)) %>%
  ungroup() %>%
  select(-rows_to_add)
...
#Want there to be the number of rows for corresponding to the Colnum 
```
...
haul$Species=rep_along(haul$Species,c("brown shrimp", "atlantic croaker", "southern flounder", "spot", "blue crab", "atlantic menhaden")) #this repeats these species common names for the entirety of the length of the haul df
...
sel_spp_120 <- P120_bioledt %>% filter(Speciescommonname %in% select_spp) %>% select(-Frequenc,-Length,-Sex,-Maturity,-Latitude, Longitude) %>% group_by(Speciescommonname,Control1)%>% mutate(id=row_number()) %>% filter(id==1) %>%ungroup()%>%as.data.frame()

colnames(P120_bioledt)
sel_spp_120edt = sel_spp_120 %>% select(Control1,Date,Speciescommonname,Colnum, Photoperiod, Season, Ym_date)
#Add in juveniles here 
...
p120_CPUE <- p120_CPUE %>% left_join(names_join)
