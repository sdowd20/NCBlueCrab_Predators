...
```{r}
speciescount <- P120_bioledt %>% group_by(Speciescommonname, Date, Location) %>% drop_na(Colnum) %>% distinct(Colnum) %>% ungroup() %>% group_by(Speciescommonname) %>% summarise(sum_colnum= sum(Colnum)) %>% filter(Speciescommonname %in% c("northern brown shrimp", "bluefish", "atlantic croaker", "southern flounder", "red drum", "black drum", "blue catfish", "gizzard shad", "american shad", "atlantic thread herring", "cownose ray", "blue crab"))
#Remove cownose ray, Atlantic thread herring, and black drum b/c n < 100
select_spp <- c("northern brown shrimp", "bluefish", "atlantic croaker", "southern flounder", "red drum", "blue catfish", "gizzard shad", "american shad", "blue crab")

p120_core <- P120_bioledt %>% filter(Core==1|Core==2) 
p120_core$Location <- ifelse(substr(p120_core$Location, 1, 1) == "0", substr(p120_core$Location, 2, nchar(p120_core$Location)), p120_core$Location) #substr: check if first integer in Location column = 0, if it does than extract substring at second character (remove leading 0), need nchar to determine length of each element, did this because there were repeating locations in Control1 that appeared to be same location (528410300 vs. 0528410300)

haul <- p120_core %>% group_by(Date, Location) %>% mutate(ID=row_number()) %>% filter(ID==1) %>% as.data.frame() #row every haul (6797 variables whereas Lela had 8099)
length(unique(haul$Control1))
nrow(haul)
haul_df <- haul %>% dplyr::select(Control1, Program, Year, Month, Day, Date, Location, Nbrrec3, Station, Time, Duration, Sedsize, Depth, Stemp, Btemp, Ssal, Bsal, Sdo, Bdo, Lat_dd, Long_dd, Core)

p120_coreedt <- p120_core %>% select(-Lat_dd, -Long_dd) %>% group_by(Control1, Speciescommonname) %>% mutate(id= row_number()) %>% filter(id== 1) %>% ungroup() %>% dplyr::select(Control1, Date, Speciescommonname, Colnum) %>% filter(Speciescommonname %in% select_spp)

#7062 vs. 6797 ControlID for p120_core vs. haul
length(unique(p120_coreedt$Control1))


if (!(select_spp %in% p120_coreedt$Speciescommonname & target_ID %in% df$ID)) {
  # Add the target species with zero value
  new_row <- data.frame(ID = target_ID, Species = target_species, Value = 0)
  df <- rbind(df, new_row)
}


unique_ids <- unique(p120_coreedt$Control1)
for(id in unique_ids){
if (!(select_spp %in% p120_coreedt$Speciescommonname[p120_coreedt$Control1 %in% id])){
 new_row <- data.frame(Control1 = id, Colnum = NA, Species = select_spp)
    df <- rbind(p120_coreedt, new_row)
  }
}


unique_ids <- unique(df$Control1_ID)
for (id in unique_ids) {
  for (species in target_species) {
    if (!(species %in% df$Species[df$Control1_ID == id])) {
      # Add a new row for the target species with zero value
      new_row <- data.frame(Control1_ID = id, Fish_Tow = NA, Species = species)
      df <- rbind(df, new_row)
    }
  }
}
