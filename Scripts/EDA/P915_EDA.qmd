...
p915_bioln <- p915_bioln %>% filter(!Area %in% "miss", Speciescommonname %in% "bonnethead hammerhead")


p915_nums <- p915_bioln %>% filter(Year > 2008, Month == 5| Month==6| Month==9) %>% group_by(Speciescommonname, Month) %>% drop_na(Colnum) %>% summarise(Sum_colnum= sum(Colnum)) %>% mutate(Survey= "P915")
