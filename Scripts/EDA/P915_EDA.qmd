...
p915_CPUEold <- read.csv("Data/P915/Finalized/p915clean_new.csv")
...
p915_bioln_sel <- p915_bioln %>% dplyr::select(Control1, Date, Month, Year, Ym_date, Location, Total.species.sobserved.in.set, Station, Quad, Repsize, Sciname, Speciescommonname, Colnum, Colwgt, Samnum, Samwgt, Subnum, Subwgt, Frequenc, Length, Weight, Maturity, Age, Sample, Latitude, Longitude)

summary(is.na(p915_bioln_sel)) #2 NAs in Colnum, 19 in Quad, 343 in Colwgt, 396 in Length, a lot in weight, maturity and age 

p915_bioln_sel %>% drop_na(Length) %>% group_by(Speciescommonname) %>% summarise(mean_l = mean(Length), min_l = min(Length), max_l= max(Length))
