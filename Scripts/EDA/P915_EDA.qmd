...
p915_bioln <- p915_bioln %>% filter(between(Year, 2008, 2020)) %>% filter(!Area %in% "miss")
...
ggplot() + geom_histogram(data = subset(p915_CPUEold, p915_CPUEold$Colnum > 0), stat="count", aes(x=Season)) + standard_theme + facet_wrap(~Species)
```
...
p915_bioln_sel %>% drop_na(Length) %>% group_by(Speciescommonname) %>% ggplot() + geom_histogram(aes(x= Length*0.0393701)) + facet_wrap(~Speciescommonname, scale= "free") + xlab("Length (in)") + ylab("Count") + standard_theme
...
idk <- rbind(p915_bioln_sel, zero_catch)
