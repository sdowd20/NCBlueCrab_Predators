...
fine <- p195_abund_edt %>% filter(Speciescommonname %in% interest_spp) #species of interest
ggplot() + geom_histogram(data = subset(fine, fine$Numbertotal > 0), stat="count", aes(x=Season)) + standard_theme + facet_wrap(~Speciescommonname, scales= "free")
