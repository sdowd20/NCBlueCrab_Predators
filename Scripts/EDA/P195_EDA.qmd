...
This is the subsample protocol for species in general: 
  1) If species are present in low #s (< 30 individuals) with one or multiple size classes, all individuals are processed as one sample. If a target species, all individuals are measured.
  2) For species present in low numbers with multiple size classes (< 30 individuals per size class but more than 30 overall), each size class is processed as a separate sample
  3) For species present in moderate numbers (< 100 individuals) with one size class, all individuals are processed as one samples. If a species is on the target species list, a sub-sample of 30-60 individuals is measured. 
  4) For species in moderate #s with multiple size classes (< 100 individuals per size class), each size class is processed separately. 
  5 and 6) ""
...
t23<- p195_lengthfreq %>% filter(Speciescommonname %in% "blue crab") %>% select(Sex)
```
...
haul$Species=rep_along(haul$Species,c("brown shrimp", "bluefish", "atlantic croaker", "southern flounder", "spot", "blue crab", "atlantic menhaden", "juv blue crab", "adult blue crab"))
...
haul_final=left_join(haul_df_all,haul,by="Eventname")%>%as.data.frame() #9*2012= 18108, it worked
...
sel_spp_195joinedt <- sel_spp_195join %>% group_by(Speciescommonname, Eventname) %>% mutate(Numbertotal_new = sum(Numbertotal)) %>% distinct(Numbertotal_new, .keep_all= TRUE) #There are some duplicates for atlantic menhaden and brown shrimp and one for spot for Eventnames, Dan said perhaps because of separating into diseased vs. not, goes from 11806 to 11620 rows (there were 186 rows w/ id= 2)
sel_spp_195joinedt2 <- sel_spp_195joinedt %>% group_by(Speciescommonname, Eventname) %>% mutate(id= row_number()) %>% filter(id==1) %>%ungroup()%>%as.data.frame() %>% select(Eventname,Date,Speciescommonname, Numbertotal_new) 
