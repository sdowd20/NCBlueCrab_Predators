...
```{r}
length_test <- read.csv("~/Desktop/test.bc.csv")
length_test <- length_test %>% rename(Length= LENGTH) 
length_test %>% drop_na(Length) %>% summarise(min_l= min(Length), max_l= max(Length), mean_l = mean(Length), sd_l= sd(Length)) #def should divide up blue crabs 
...
Length data pulled: 
DataType:	Length Frequency
AreaLocation:	NEUSE RIVER;PAMLICO RIVER;PAMLICO SOUND; EAST OF BLUFF SHOAL;PAMLICO SOUND; WEST OF BLUFF SHOAL;PUNGO RIVER
CommonName:	ATLANTIC CROAKER;ATLANTIC MENHADEN;BLUE CRAB;BLUEFISH;NORTHERN BROWN SHRIMP;NORTHERN KINGFISH;SOUTHERN FLOUNDER;SOUTHERN KINGFISH;SPOT;SUMMER FLOUNDER
Season:	Summer (June|July|August);Fall (September|October|November)
