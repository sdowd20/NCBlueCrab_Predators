...
#visualize how many stations are sampled at each location and depth over time
...
p195_abund_edt %>% group_by(Date, Location, Depthzone) %>% mutate(ID=row_number()) %>% filter(ID==1) %>% ggplot() + geom_histogram(aes(x= Ym_date), stat= "count") + facet_wrap(~Location + Depthzone) + standard_theme 
