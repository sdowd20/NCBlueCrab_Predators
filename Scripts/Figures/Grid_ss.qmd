...
#there are common gridIDs
# t <- CPUE_grid_count_avg_edt %>% filter(Survey %in% "P915")
# t2 <- CPUE_grid_count_avg_edt %>% filter(Survey %in% "P120")
# common_gridIDs <- intersect(t$gridID, t2$gridID)
```
...
```{r}
df_count_wide_both %>% filter(reddrumP915 > 0)
#Red drum: 181, black drum: 112, southern kingfish: 68

data <- data.frame(species = c("Red drum", "Black drum", "Southern kingfish"), count = c(181, 112, 68))

ggplot(data, aes(x = "", y = count, fill = species)) + geom_bar(stat = "identity", width = 0.8) + standard_theme + labs(x= "0.12° x 0.12°", y= "Number of positive encounters", fill= "Species") + scale_fill_manual(values= c("#0072B2", "#009E73", "#CC6666"), name= "Species")
