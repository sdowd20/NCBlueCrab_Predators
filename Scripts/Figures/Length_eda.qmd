...
#Check
p915_gg %>% group_by(Speciescommonname) %>% summarize(sum= sum(Colnum))
...
```{r}
ggplot(data= p915_length_toplot) + geom_histogram(aes(x= Length)) + standard_theme + facet_wrap(~Speciescommonname, scales= "free")

p915_gg %>% group_by(Speciescommonname) %>% summarize(count= sum(Colnum))
