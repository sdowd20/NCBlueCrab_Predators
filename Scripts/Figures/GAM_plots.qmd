...
sm %>%
  ...
  ggplot() + geom_rug(aes(x= {{predictor}}), data= pr) + geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, x = {{predictor}}), alpha = 0.2) + geom_point(aes(x = {{predictor}}, y = {{predictor2}}), data = pr, cex = 1.5, colour = "steelblue3")+ geom_line(aes(x = {{predictor}}, y = est), lwd = 1.2) + labs(y = "Partial effect", title = predictor3) + geom_point(aes(x= {{predictor}}, y= {{predator}}), data=df) + standard_theme + xlab(xlabel) + labs(title=NULL)
}

plot("s(avgssal)", avgssal, `s(avgssal)`, "s(avgssal)", reddrumP915, "Average surface salinity")
plot("s(avgstemp)", avgstemp, `s(avgstemp)`, "s(avgstemp)", reddrumP915, "Average surface temperature")
plot("s(avgdepth)", avgdepth, `s(avgdepth)`, "s(avgdepth)", reddrumP915, "Average depth")
plot("s(avgsdo)", avgsdo, `s(avgsdo)`, "s(avgsdo)", reddrumP915, "Average surface dissolved oxygen")
plot("s(NoFishRest)", NoFishRest, `s(NoFishRest)`, "s(NoFishRest)", reddrumP915, "NoFishRest")
plot("s(logsmallbluecrabP120)", logsmallbluecrabP120, `s(logsmallbluecrabP120)`, "s(logsmallbluecrabP120)", reddrumP915, "Small blue crab CPUE") 
```
