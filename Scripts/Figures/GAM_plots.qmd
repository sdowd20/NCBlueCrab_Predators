...
ggsave("/users/sallydowd/Google Drive/My Drive/Research/Ch1Data/Figures/partial_rd.jpeg", width= 8, height= 6)
```
...
ggsave("/users/sallydowd/Google Drive/My Drive/Research/Ch1Data/Figures/partial_sk.jpeg", width= 8, height= 6)
```
...
annotate_figure(t3, left= textGrob("Partial effect", rot=90), fig.lab= "C)", top= "blackdrumP915 ~s(avgdepth)+s(avgssal)+s(avgstemp)+s(avgsdo)+s(NoFishRest)+factor(Yearfactor)+s(logblackdrumP915forageP120)")
ggsave("/users/sallydowd/Google Drive/My Drive/ResearchCh1Data/Figures/partial_bd.jpeg", width= 8, height= 6)
```
