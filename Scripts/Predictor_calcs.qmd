...
DPA_aA <- grid_edt %>% left_join(DPA_aA, by= "gridID") %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("DPA_aA"= "column")

###ITP Management Area: Area overlap, take area with largest overlap
ITP <- st_intersection(ITP_Management, grid_edt) %>% cbind(as.data.frame(st_area(.))) %>% select(gridID, area, st_area...) %>% group_by(gridID) %>% mutate(st_area...= as.numeric(st_area...)) %>% filter(st_area... %in% max(st_area...)) %>% st_drop_geometry() %>% select(-st_area...) %>% rename("ITP"= "area")
ITP <- grid_edt %>% left_join(ITP, by= "gridID") %>% st_drop_geometry()

##Restricted 
###Shrimp trawling prohibited
ShrimpTrawl_pA <- perc_A(grid_edt, ShrimpTrawl)
ShrimpTrawl_pA <- grid_edt %>% left_join(ShrimpTrawl_pA, by= "gridID") %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("ShrimpTrawl_pA"= "column")

###Trawling, dredging prohibited
TrawlDredge_p_edt <- TrawlDredge_p %>% st_union()
TrawlDredge_pA <- perc_A(grid_edt, TrawlDredge_p_edt)
TrawlDredge_pA <- grid_edt %>% left_join(TrawlDredge_pA, by= "gridID") %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("TrawlDredge_pA"= "column")

###Passive gear prohibited
GillPound_pA <- perc_A(GillPound_p_edt, grid_edt)
GillPound_pA <- grid_edt %>% left_join(GillPound_pA, by= "gridID") %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("GillPound_pA"= "column")

###Total restricted area
RestrictedArea_pA <- perc_A(RestrictedArea, grid_edt)
RestrictedArea_pA <- grid_edt %>% left_join(RestrictedArea_pA, by= "gridID") %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("RestrictedArea_pA"= "column")

#Waterbody
Waterbodi <- All_waterbodies %>% st_union()
Waterbody_A <- perc_A(grid_edt, Waterbodi) #this worked! QGIS
Waterbody_A <- grid_edt %>% left_join(Waterbody_A) %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("Waterbody_A"= "column")
...
ShorelineType <- ShorelineType %>% left_join(ESE, by= "row_num") %>% select(gridID, SHORE_NAM) %>% rename("ShorelineType"= "SHORE_NAM") %>% st_drop_geometry() #21 differences in 120 gridIDs in common b/w old dataset and new, might be due to different calculation distances 
```
...
##Organize datasets: 
datasets_Sedsize <- CPUE_all %>% dplyr::select(Latitude, Longitude, Sedsize_new) %>% drop_na(Sedsize_new)
...
Sedsize_common <- datasets_Sedsize %>% group_by(gridID) %>% summarise(Sedsize_common = names(which.max(table(Sedsize_new)))) %>% drop_na(gridID) #be careful here with dropping gridIDs 
...
merged_df4 <- merged_df3 %>% mutate(FishingAll_num= rowSums(merged_df3[c("ShrimpTrawl_aA", "TrawlDredge_aA", "GillPound_aA", "DPA_aA")] > 0), NoFishRest= Waterbody_A - RestrictedArea_pA)

#TrawlDredge_aA is off and RestrictedArea

#Remove certain GridIDs
