...
df_new <- st_intersection(df1, df2) %>% cbind(as.data.frame(st_area(.))) %>% select(gridID, st_area..., geometry) %>% mutate(st_area...= as.numeric(st_area...)) %>% group_by(gridID) %>% mutate(st_area_km= st_area.../1000000, sum_area_km2= sum(st_area_km), column= (sum_area_km2/grid_A)*100) %>% distinct(gridID, .keep_all = TRUE) %>% st_drop_geometry()
return(df_new)
}
...
ITP <- st_intersection(ITP_Management, grid_edt) %>% cbind(as.data.frame(st_area(.))) %>% select(gridID, area, st_area...) %>% group_by(gridID) %>% mutate(st_area...= as.numeric(st_area...)) %>% filter(st_area... %in% max(st_area...)) %>% st_drop_geometry()
...
Waterbodi_A <- 
Waterbody_A <- st_intersection(grid_edt, Waterbodi) %>% cbind(as.data.frame(st_area(.))) %>% select(gridID, st_area..., geometry) %>% mutate(st_area...= as.numeric(st_area...)) %>% group_by(gridID) %>% mutate(st_area_km= st_area.../1000000, sum_area_km2= sum(st_area_km), column= (sum_area_km2/grid_A)*100)
Waterbody_A %>% filter(column > 100)
st_make_valid(Waterbody_A)
st_area(Waterbody_A)
Waterbody_A <- perc_A(ShrimpTrawl_p, grid_edt)
...
tester <- st_intersection(All_waterbodies, grid_edt)
