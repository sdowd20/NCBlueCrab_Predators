...
ITP <- st_intersection(ITP_Management, grid_edt) %>% cbind(as.data.frame(st_area(.))) %>% select(gridID, area, st_area...) %>% group_by(gridID) %>% mutate(st_area...= as.numeric(st_area...)) %>% filter(st_area... %in% max(st_area...)) %>% st_drop_geometry() %>% select(-st_area...)

##Restricted 
###Shrimp trawling prohibited
Shrimp_trawl_pA <- perc_A(ShrimpTrawl_p, grid_edt)
Shrimp_trawl_pA <- grid_edt %>% left_join(Shrimp_trawl_pA, by= "gridID") %>% replace(is.na(.), 0) %>% arrange(gridID) %>% st_drop_geometry() %>% select(gridID, column) %>% rename("Shrimp_trawl_pA"= "column")

###Trawling, dredging prohibited
TrawlDredge_pA <- perc_A(TrawlingDredging_p, grid_edt)
...
##Distance to SAV
options(scipen = 999)
SAV_km <- nearest_dist(sf_cent, SAV_edt) %>% rename("SAV_km"= "dist_km") %>% st_drop_geometry()
options(scipen = 0)
#These are pretty far off from original calculations, this makes sense b/c using habitat mosaic throughout time instead of 2012-2016
