...
df_CPUE_wide_train =df_CPUE_wide[picked,]
...
Helpful articles:
-smoothers and k: https://stats.stackexchange.com/questions/243367/smoothing-methods-for-gam-in-mgcv-package
-choosing k: https://stat.ethz.ch/R-manual/R-devel/library/mgcv/html/choose.k.html
-stack overflow: https://stackoverflow.com/questions/60078003/gam-in-r-is-it-a-spline-model-with-automated-knots-selection
...
prop_0 <- 100*sum(df_CPUE_wide$red_drum == 0)/nrow(df_CPUE_wide) #definitely zero inflated 
```

#### Initial models: 
```{r}
#Generalized linear model: perfect separation is happening 
flounder_glm <- glm(red_drum ~ avg_depth + avg_ssal + avg_stemp + avg_sdo + SAVDist_km + InletDist_km + NoFishRest, data = df_CPUE_wide_train, family="poisson")
...
binomial <- gam(red_drum ~  s(avg_depth) + s(avg_ssal) + s(avg_stemp) + s(avg_sdo) + s(SAVDist_km) + s(InletDist_km) + s(NoFishRest), family=binomial(link="logit"), data= df_binary_wide_train)
summary(binomial)
gam.check(binomial)

#Negative binomial: related to poisson, use for count data
binomial_neg <- gam(red_drum ~  s(avg_depth) + s(avg_ssal) + s(avg_stemp) + s(avg_sdo) + s(SAVDist_km) + s(InletDist_km) + s(NoFishRest), family= "nb", data= df_count_wide_train)
...
poisson <- gam(red_drum ~  s(avg_depth) + s(avg_ssal) + s(avg_stemp) + s(avg_sdo) + s(SAVDist_km) + s(InletDist_km) + s(NoFishRest), family=poisson(), data= df_count_wide_train)
summary(poisson)
gam.check(poisson)
autoplot.countreg(poisson)

#Gaussian GAM 
smooth_gam <- gam(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + Sedsize_common, family= gaussian(link= "identity"), data= df_CPUE_wide_train, method= "REML")
summary(smooth_gam) 
```
...
```{r}
tweedie <- gam(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(black_drum, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common), family= tw(link= "log"), data= df_CPUE_wide_train, method= "REML")
summary(tweedie)
...
#penalize for the # of knots, run w/ different number of knots 
#do something to make sure # of knots is correct 

tweedie_pred <- predict(tweedie, df_CPUE_wide_test, type = "response") %>% tibble(family = "Tweedie", pred = .) %>% mutate(red_drum= df_count_wide_test$red_drum)
```
...
delta <- gam(list(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(black_drum, bs="ts", k=5), ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(black_drum, bs="ts", k=5)), family= ziplss(), data= df_count_wide_train, method = "REML")
summary(delta)

#Combine both models into one 
inv_link <- binomial(link = "cloglog")$linkinv #inverse link function, cloglog is the link 
delta_pred <- predict(delta, df_count_wide_test, type = "link") %>% as.data.frame() %>% transmute(family = "Zero-inflated Poisson", pred = inv_link(V2) * exp(V1)) %>% mutate(red_drum= df_count_wide_test$red_drum) #transmute makes new data frame w/ only specified computations, multiply effects of each submodel 
```

#### Compare models 
```{r}
test_pred <- bind_rows(delta_pred, tweedie_pred, binomial_neg_pred) %>% 
...
test_pred %>% group_by(family) %>% summarise(rank_cor = cor.test(summer_flounder, pred, method = "spearman", exact = FALSE)$estimate) %>% ungroup()

#See how many observations are underestimated by an order of mangitude or more
test_pred %>% group_by(family) %>% summarize(n = sum(summer_flounder / pred > 10),
pct = mean(summer_flounder / pred > 10))
...
test_pred %>% group_by(family) %>% summarise(mad = mean(abs(summer_flounder - pred), na.rm = TRUE)) %>% ungroup()

summary(delta)$s.table
summary(tweedie)$s.table
summary(binomial_neg_pred)$s.table

##check the choice of k
gam.check(delta)
gam.check(tweedie)
gam.check(binomial_neg_pred)

k.check(delta)
k.check(tweedie)
k.check(binomial_neg_pred)

#Covariate plot: standard error bars, include plot of residuals 
...
tweedie_full <- gam(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(black_drum, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common), family= tw(link= "log"), data= df_CPUE_wide_train, method= "REML", trControl= train.control)
```
...
allformulas<- dredgeform(pred = "red_drum", covars = c("avg_depth", "avg_ssal", "avg_stemp", "avg_sdo", "SAVDist_km", "InletDist_km", "NoFishRest", "black_drum"))
...
train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 10) #should be 100
# Train the full model
model_full <- train(as.formula(allformulas[i]), data = df_CPUE_wide_train, method = "lm",trControl = train.control)
