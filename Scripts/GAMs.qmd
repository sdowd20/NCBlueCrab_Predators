...
any(is.na(CPUE_to_runP915_updated))

CPUE_to_runP915_updated$summer_flounder <- as.integer(CPUE_to_runP915_updated$summer_flounder)
delta <- gam(summer_flounder ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(black_drum, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common), family= ziplss(), data= CPUE_to_runP915_updated)
summary(delta)  
