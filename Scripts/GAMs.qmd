...
Helpful articles:
-Model output: https://noamross.github.io/gams-in-r-course/chapter2
...
-K-fold cross validation w/ GAMs: https://p8105.com/cross_validation.html
...
tweedie_full <- train(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(atlantic_menhaden, bs="ts", k=5) + s(atlantic_croaker, bs="ts", k=5) + s(southern_flounder, bs="ts", k=5) + s(spot, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common) + factor(ShorelineType), family= tw(link= "log"), data= df_CPUE_wide, method= "REML", trControl= train.control) #ITP terminates model, excluded 


CVgam(formula, data, nfold = 10, debug.level = 0, method = "GCV.Cp",
              printit = TRUE, cvparts = NULL, gamma = 1, seed = 29)

CVgam(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(atlantic_menhaden, bs="ts", k=5) + s(atlantic_croaker, bs="ts", k=5) + s(southern_flounder, bs="ts", k=5) + s(spot, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common) + factor(ShorelineType), family= tw(link= "log"), nfold= 10, debug.level = 0, method = "GCV.Cp",printit = TRUE, cvparts = NULL, gamma = 1, seed = 29)
...
for (i in 1:10){
  b <- gam(red_drum ~ s(avg_depth, bs="ts", k=5) + s(avg_ssal, bs="ts", k=5) + s(avg_stemp, bs="ts", k=5) + s(avg_sdo, bs="ts", k=5) + s(SAVDist_km, bs="ts", k=5) + s(InletDist_km, bs="ts", k=5) + s(NoFishRest, bs="ts", k=5) + s(atlantic_menhaden, bs="ts", k=5) + s(atlantic_croaker, bs="ts", k=5) + s(southern_flounder, bs="ts", k=5) + s(spot, bs="ts", k=5) + factor(FishingAll_num) + factor(Sedsize_common) + factor(ShorelineType), family= tw(link= "log"), method= "REML", data= df_CPUE_wide)
  
  pred_b = predict(b, newdata = df_CPUE_wide_test, type = "response")
  pred_b_ud = rep("Down", 125) 
  pred_b_ud[pred_b > 0.5] = "Up"
  
  mae <- c(mae, mean(abs(as.integer(pred_b_ud=="Up")
                         -as.integer(df_CPUE_wide_test$red_drum == "Up"))))
}
...
allformulas<- dredgeform(pred = "red_drum", covars = c("avg_depth", "avg_ssal", "avg_stemp", "avg_sdo", "SAVDist_km", "InletDist_km", "NoFishRest", "atlantic_menhaden", "atlantic_croaker", "southern_flounder", "spot", "FishingAll_num", "Sedsize_common", "ShorelineType"))
...
model_full <- train(as.formula(allformulas[i]), data = df_CPUE_wide, method = "lm",trControl = train.control)
